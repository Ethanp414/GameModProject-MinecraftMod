...
public record BlocksAttacks(
   ...
   public void disable(ServerLevel $$0, LivingEntity $$1, float $$2, ItemStack $$3) {
   ...
   public static record DamageReduction(float horizontalBlockingAngle, Optional<HolderSet<DamageType>> type, float base, float factor) {
      ...
      public float resolve(DamageSource $$0, float $$1, double $$2) {
         if ($$2 > (double)((float) (Math.PI / 180.0) * this.horizontalBlockingAngle)) {
            return 0.0F;
         } else {
            return this.type.isPresent() && !((HolderSet)this.type.get()).contains($$0.typeHolder())
               ? 0.0F
               : Mth.clamp(this.base + this.factor * $$1, 0.0F, $$1);
         }
      }
   }
