...
public class RecipeManager extends SimplePreparableReloadListener<RecipeMap> implements RecipeAccess {
   ...
   private static final Map<ResourceKey<RecipePropertySet>, RecipeManager.IngredientExtractor> RECIPE_PROPERTY_SETS = Map.of(
      ...
      (RecipeManager.IngredientExtractor)$$0 -> $$0 instanceof SmithingRecipe $$1 ? $$1.additionIngredient() : Optional.empty(),
      RecipePropertySet.SMITHING_BASE,
      (RecipeManager.IngredientExtractor)$$0 -> $$0 instanceof SmithingRecipe $$1 ? Optional.of($$1.baseIngredient()) : Optional.empty(),
      RecipePropertySet.SMITHING_TEMPLATE,
      (RecipeManager.IngredientExtractor)$$0 -> $$0 instanceof SmithingRecipe $$1 ? $$1.templateIngredient() : Optional.empty(),
   ...
   protected RecipeMap prepare(ResourceManager $$0, ProfilerFiller $$1) {
      SortedMap<ResourceLocation, Recipe<?>> $$2 = new TreeMap();
      ...
      $$2.forEach(($$1x, $$2x) -> {
         ResourceKey<Recipe<?>> $$3xx = ResourceKey.create(Registries.RECIPE, $$1x);
         RecipeHolder<?> $$4 = new RecipeHolder($$3xx, $$2x);
         $$3.add($$4);
      });
   ...
   public void finalizeRecipeLoading(FeatureFlagSet $$0) {
      ...
      this.recipes.values().forEach($$3 -> {
         ...
         if (!$$4.isSpecial() && $$4.placementInfo().isImpossibleToPlace()) {
            LOGGER.warn("Recipe {} can't be placed due to empty ingredients and will be ignored", $$3.id().location());
         } else {
            $$2.forEach($$1xx -> $$1xx.accept($$4));
            if ($$4 instanceof StonecutterRecipe $$5 && isIngredientEnabled($$0, $$5.input()) && $$5.resultDisplay().isEnabled($$0)) {
               $$1.add(new SelectableRecipe.SingleInputEntry($$5.input(), new SelectableRecipe($$5.resultDisplay(), Optional.of($$3))));
            }
         }
      });
      this.propertySets = (Map)$$2.stream().collect(Collectors.toUnmodifiableMap($$0x -> $$0x.key, $$1x -> $$1x.asPropertySet($$0)));
      this.stonecutterRecipes = new SelectableRecipe.SingleInputSet<>($$1);
      this.allDisplays = unpackRecipeInfo(this.recipes.values(), $$0);
      this.recipeToDisplay = (Map)this.allDisplays
         .stream()
         .collect(Collectors.groupingBy($$0x -> $$0x.parent.id(), IdentityHashMap::new, Collectors.toList()));
   }
   ...
   private static boolean isIngredientEnabled(FeatureFlagSet $$0, Ingredient $$1) {
      return $$1.items().allMatch($$1x -> ((Item)$$1x.value()).isEnabled($$0));
   }
   ...
   private <T extends Recipe<?>> RecipeHolder<T> byKeyTyped(RecipeType<T> $$0, ResourceKey<Recipe<?>> $$1) {
      ...
      return $$2 != null && $$2.value().getType().equals($$0) ? $$2 : null;
   }
   ...
   public RecipePropertySet propertySet(ResourceKey<RecipePropertySet> $$0) {
      return (RecipePropertySet)this.propertySets.getOrDefault($$0, RecipePropertySet.EMPTY);
   }
   ...
   public RecipeManager.ServerDisplayInfo getRecipeFromDisplay(RecipeDisplayId $$0) {
      int $$1 = $$0.index();
      return $$1 >= 0 && $$1 < this.allDisplays.size() ? (RecipeManager.ServerDisplayInfo)this.allDisplays.get($$1) : null;
   }

   public void listDisplaysForRecipe(ResourceKey<Recipe<?>> $$0, Consumer<RecipeDisplayEntry> $$1) {
      List<RecipeManager.ServerDisplayInfo> $$2 = (List)this.recipeToDisplay.get($$0);
      if ($$2 != null) {
   ...
   protected static RecipeHolder<?> fromJson(ResourceKey<Recipe<?>> $$0, JsonObject $$1, HolderLookup.Provider $$2) {
      Recipe<?> $$3 = Recipe.CODEC.parse($$2.createSerializationContext(JsonOps.INSTANCE), $$1).getOrThrow(JsonParseException::new);
   ...
   public static <I extends RecipeInput, T extends Recipe<I>> RecipeManager.CachedCheck<I, T> createCheck(final RecipeType<T> $$0) {
      return new RecipeManager.CachedCheck<I, T>() {
         ...
         public Optional<RecipeHolder<T>> getRecipeFor(I $$0x, ServerLevel $$1) {
            ...
            if ($$3.isPresent()) {
               RecipeHolder<T> $$4 = (RecipeHolder)$$3.get();
