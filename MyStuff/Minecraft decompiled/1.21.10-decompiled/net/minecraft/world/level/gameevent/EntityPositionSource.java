...
public class EntityPositionSource implements PositionSource {
   ...
   public Optional<Vec3> getPosition(Level $$0) {
      ...
      return this.entityOrUuidOrId.left().map($$0x -> $$0x.position().add(0.0, (double)this.yOffset, 0.0));
   }

   private void resolveEntity(Level $$0) {
      ((Optional)this.entityOrUuidOrId
            .map(
               Optional::of, $$1 -> Optional.ofNullable((Entity)$$1.map($$1x -> $$0 instanceof ServerLevel $$2 ? $$2.getEntity($$1x) : null, $$0::getEntity))
            ))
         .ifPresent($$0x -> this.entityOrUuidOrId = Either.left($$0x));
   }

   public UUID getUuid() {
      return this.entityOrUuidOrId.map(Entity::getUUID, $$0 -> $$0.map(Function.identity(), $$0x -> {
            throw new RuntimeException("Unable to get entityId from uuid");
         }));
   }

   private int getId() {
      return this.entityOrUuidOrId.map(Entity::getId, $$0 -> $$0.map($$0x -> {
