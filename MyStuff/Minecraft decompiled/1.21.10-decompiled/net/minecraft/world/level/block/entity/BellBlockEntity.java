...
public class BellBlockEntity extends BlockEntity {
   ...
   private static void tick(Level $$0, BlockPos $$1, BlockState $$2, BellBlockEntity $$3, BellBlockEntity.ResonationEndAction $$4) {
      if ($$3.shaking) {
         ++$$3.ticks;
      }
   ...
   private static void showBellParticles(Level $$0, BlockPos $$1, List<LivingEntity> $$2) {
      ...
      $$2.stream()
         .filter($$1x -> isRaiderWithinRange($$1, $$1x))
         .forEach(
            $$4x -> {
               float $$5 = 1.0F;
               double $$6 = Math.sqrt(
                  ($$4x.getX() - (double)$$1.getX()) * ($$4x.getX() - (double)$$1.getX())
                     + ($$4x.getZ() - (double)$$1.getZ()) * ($$4x.getZ() - (double)$$1.getZ())
               );
               double $$7 = (double)((float)$$1.getX() + 0.5F) + 1.0 / $$6 * ($$4x.getX() - (double)$$1.getX());
               double $$8 = (double)((float)$$1.getZ() + 0.5F) + 1.0 / $$6 * ($$4x.getZ() - (double)$$1.getZ());
               int $$9 = Mth.clamp(($$4 - 21) / -2, 3, 15);
      
               for(int $$10 = 0; $$10 < $$9; ++$$10) {
                  int $$11 = $$3.addAndGet(5);
                  $$0.addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, $$11), $$7, (double)((float)$$1.getY() + 0.5F), $$8, 0.0, 0.0, 0.0);
               }
            }
         );
   }
