...
public class TestInstanceBlockEntity extends BlockEntity implements BeaconBeamOwner, BoundingBoxRenderable {
   ...
   public Component getTestName() {
      return (Component)this.test().map($$0 -> Component.literal($$0.location().toString())).orElse(INVALID_TEST_NAME);
   }
   ...
   protected void loadAdditional(ValueInput $$0) {
      $$0.read("data", TestInstanceBlockEntity.Data.CODEC).ifPresent(this::set);
      this.errorMarkers.clear();
      this.errorMarkers.addAll((Collection)$$0.read("errors", TestInstanceBlockEntity.ErrorMarker.LIST_CODEC).orElse(List.of()));
   }
   ...
   public List<BeaconBeamOwner.Section> getBeamSections() {
      return switch(this.data.status().ordinal()) {
         case 0 -> BEAM_CLEARED;
         case 1 -> BEAM_RUNNING;
         case 2 -> this.errorMessage().isEmpty()
         ? BEAM_SUCCESS
   ...
   public void resetTest(Consumer<Component> $$0) {
      ...
      if ($$1) {
         $$0.accept(Component.translatable("test_instance_block.reset_success", this.getTestName()).withStyle(ChatFormatting.GREEN));
      }
   ...
   public Optional<ResourceLocation> saveTest(Consumer<Component> $$0) {
      ...
      if ($$1.isPresent()) {
         $$2 = Optional.of(((GameTestInstance)((Holder.Reference)$$1.get()).value()).structure());
      } else {
         $$2 = this.test().map(ResourceKey::location);
      }

      if ($$2.isEmpty()) {
         BlockPos $$4 = this.getBlockPos();
         $$0.accept(Component.translatable("test_instance_block.error.unable_to_save", $$4.getX(), $$4.getY(), $$4.getZ()).withStyle(ChatFormatting.RED));
         return $$2;
      } else {
         Level var5 = this.level;
         if (var5 instanceof ServerLevel $$5) {
            StructureBlockEntity.saveStructure(
               $$5, (ResourceLocation)$$2.get(), this.getStructurePos(), this.getSize(), this.ignoreEntities(), "", true, List.of(Blocks.AIR)
            );
         }
   ...
   public boolean exportTest(Consumer<Component> $$0) {
      ...
      if (!$$1.isEmpty()) {
         ...
         if (var4 instanceof ServerLevel $$2) {
            return export($$2, (ResourceLocation)$$1.get(), $$0);
         }
      }
   ...
   public void runTest(Consumer<Component> $$0) {
      ...
      if ($$3 instanceof ServerLevel $$1) {
         ...
         BlockPos $$4 = this.getBlockPos();
         if (var7.isEmpty()) {
            $$0.accept(Component.translatable("test_instance_block.error.no_test", $$4.getX(), $$4.getY(), $$4.getZ()).withStyle(ChatFormatting.RED));
         } else if (!this.placeStructure()) {
            $$0.accept(Component.translatable("test_instance_block.error.no_test_structure", $$4.getX(), $$4.getY(), $$4.getZ()).withStyle(ChatFormatting.RED));
         } else {
            this.clearErrorMarkers();
            GameTestTicker.SINGLETON.clear();
            FailedTestTracker.forgetFailedTests();
            $$0.accept(Component.translatable("test_instance_block.starting", ((Holder.Reference)var7.get()).getRegisteredName()));
            GameTestInfo $$5 = new GameTestInfo((Holder.Reference<GameTestInstance>)var7.get(), this.data.rotation(), $$1, RetryOptions.noRetries());
            $$5.setTestBlockPos($$4);
            GameTestRunner $$6 = GameTestRunner.Builder.fromInfo(List.of($$5), $$1).build();
   ...
   public boolean placeStructure() {
      Level $$1 = this.level;
      if ($$1 instanceof ServerLevel $$0) {
         Optional<StructureTemplate> $$1x = this.data.test().flatMap($$1x -> getStructureTemplate($$0, $$1x));
         if ($$1x.isPresent()) {
            this.placeStructure($$0, (StructureTemplate)$$1x.get());
            return true;
         }
      }
   ...
   public static enum Status implements StringRepresentable {
      ...
      private static final IntFunction<TestInstanceBlockEntity.Status> ID_MAP = ByIdMap.continuous(
         $$0 -> $$0.index, values(), ByIdMap.OutOfBoundsStrategy.ZERO
      );
      ...
      public static TestInstanceBlockEntity.Status byIndex(int $$0) {
         return (TestInstanceBlockEntity.Status)ID_MAP.apply($$0);
      }
   }
}
