...
public class TrialSpawnerStateData {
   ...
   public void tryDetectPlayers(ServerLevel $$0, BlockPos $$1, TrialSpawner $$2) {
      ...
      if (!$$3) {
         if (!$$2.getState().equals(TrialSpawnerState.COOLDOWN) || !$$2.isOminous()) {
            List<UUID> $$4 = $$2.getPlayerDetector().detect($$0, $$2.getEntitySelector(), $$1, (double)$$2.getRequiredPlayerRange(), true);
            boolean $$7;
            if (!$$2.isOminous() && !$$4.isEmpty()) {
               Optional<Pair<Player, Holder<MobEffect>>> $$6 = findPlayerWithOminousEffect($$0, $$4);
               $$6.ifPresent($$3x -> {
                  Player $$4xx = (Player)$$3x.getFirst();
                  if ($$3x.getSecond() == MobEffects.BAD_OMEN) {
                     transformBadOmenIntoTrialOmen($$4xx);
                  }

                  $$0.levelEvent(3020, BlockPos.containing($$4xx.getEyePosition()), 0);
                  $$2.applyOminous($$0, $$1);
               });
               $$7 = $$6.isPresent();
            } else {
            ...
            if (!$$2.getState().equals(TrialSpawnerState.COOLDOWN) || $$7) {
               ...
               List<UUID> $$9 = $$8 ? $$4 : $$2.getPlayerDetector().detect($$0, $$2.getEntitySelector(), $$1, (double)$$2.getRequiredPlayerRange(), false);
   ...
   public void resetAfterBecomingOminous(TrialSpawner $$0, ServerLevel $$1) {
      this.currentMobs.stream().map($$1::getEntity).forEach($$1x -> {
         if ($$1x != null) {
            ...
            $$1x.remove(Entity.RemovalReason.DISCARDED);
         }
      });
      ...
      this.totalMobsSpawned = 0;
      this.currentMobs.clear();
      this.nextMobSpawnsAt = $$1.getGameTime() + (long)$$0.ominousConfig().ticksBetweenSpawn();
      $$0.markUpdated();
   ...
   protected SpawnData getOrCreateNextSpawnData(TrialSpawner $$0, RandomSource $$1) {
      if (this.nextSpawnData.isPresent()) {
         ...
         this.nextSpawnData = Optional.of((SpawnData)$$3.orElseGet(SpawnData::new));
         $$0.markUpdated();
         return (SpawnData)this.nextSpawnData.get();
      }
   }
   ...
   public CompoundTag getUpdateTag(TrialSpawnerState $$0) {
      ...
      this.nextSpawnData.ifPresent($$1x -> $$1.store("spawn_data", SpawnData.CODEC, $$1x));
      return $$1;
   }
   ...
   public static record Packed(
      ...
      public static final MapCodec<TrialSpawnerStateData.Packed> MAP_CODEC = RecordCodecBuilder.mapCodec(
         $$0 -> $$0.group(
                  ...
                  Codec.LONG.lenientOptionalFieldOf("cooldown_ends_at", Long.valueOf(0L)).forGetter(TrialSpawnerStateData.Packed::cooldownEndsAt),
                  Codec.LONG.lenientOptionalFieldOf("next_mob_spawns_at", Long.valueOf(0L)).forGetter(TrialSpawnerStateData.Packed::nextMobSpawnsAt),
