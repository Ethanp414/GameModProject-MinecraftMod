...
public class PathFinder {
   ...
   public Path findPath(PathNavigationRegion $$0, Mob $$1, Set<BlockPos> $$2, float $$3, int $$4, float $$5) {
      ...
      if ($$6 == null) {
         ...
         Map<Target, BlockPos> $$7 = (Map)$$2.stream()
            .collect(
               Collectors.toMap($$0x -> this.nodeEvaluator.getTarget((double)$$0x.getX(), (double)$$0x.getY(), (double)$$0x.getZ()), Function.identity())
            );
   ...
   private Path findPath(Node $$0, Map<Target, BlockPos> $$1, float $$2, int $$3, float $$4) {
      ...
      int $$9 = 0;
      Set<Target> $$10 = Sets.<Target>newHashSetWithExpectedSize($$6.size());
      int $$11 = (int)((float)this.maxVisitedNodes * $$4);
      ...
      while(!this.openSet.isEmpty()) {
         ...
         for(Target $$13 : $$6) {
            if ($$12.distanceManhattan($$13) <= (float)$$3) {
               $$13.setReached();
               $$10.add($$13);
            }
         }
      ...
      Optional<Path> $$19 = !$$10.isEmpty()
         ? $$10.stream().map($$1x -> this.reconstructPath($$1x.getBestNode(), (BlockPos)$$1.get($$1x), true)).min(Comparator.comparingInt(Path::getNodeCount))
         : $$6.stream()
            .map($$1x -> this.reconstructPath($$1x.getBestNode(), (BlockPos)$$1.get($$1x), false))
      ...
      if ($$19.isEmpty()) {
         ...
         Path $$20 = (Path)$$19.get();
         if ($$7) {
            $$20.setDebug(this.openSet.getHeap(), (Node[])$$8.toArray($$0x -> new Node[$$0x]), $$6);
         }
   ...
   private Path reconstructPath(Node $$0, BlockPos $$1, boolean $$2) {
      List<Node> $$3 = Lists.<Node>newArrayList();
      Node $$4 = $$0;
      $$3.add(0, $$0);
