...
public class CocoaDecorator extends TreeDecorator {
   ...
   public void place(TreeDecorator.Context $$0) {
      ...
      if (!($$1.nextFloat() >= this.probability)) {
         ...
         if (!$$2.isEmpty()) {
            int $$3 = ((BlockPos)$$2.getFirst()).getY();
            $$2.stream()
               .filter($$1x -> $$1x.getY() - $$3 <= 2)
               .forEach(
                  $$2x -> {
                     for(Direction $$3xx : Direction.Plane.HORIZONTAL) {
                        if ($$1.nextFloat() <= 0.25F) {
                           Direction $$4 = $$3xx.getOpposite();
                           BlockPos $$5 = $$2x.offset($$4.getStepX(), 0, $$4.getStepZ());
                           if ($$0.isAir($$5)) {
                              $$0.setBlock(
                                 $$5,
                                 Blocks.COCOA.defaultBlockState().setValue(CocoaBlock.AGE, Integer.valueOf($$1.nextInt(3))).setValue(CocoaBlock.FACING, $$3xx)
                              );
                           }
                        }
                     }
