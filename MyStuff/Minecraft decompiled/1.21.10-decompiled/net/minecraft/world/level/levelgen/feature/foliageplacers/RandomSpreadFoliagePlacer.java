...
public class RandomSpreadFoliagePlacer extends FoliagePlacer {
   public static final MapCodec<RandomSpreadFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(
      $$0 -> foliagePlacerParts($$0)
            .and(
               $$0.group(
                  IntProvider.codec(1, 512).fieldOf("foliage_height").forGetter($$0x -> $$0x.foliageHeight),
                  Codec.intRange(0, 256).fieldOf("leaf_placement_attempts").forGetter($$0x -> $$0x.leafPlacementAttempts)
               )
            )
            .apply($$0, RandomSpreadFoliagePlacer::new)
   );
