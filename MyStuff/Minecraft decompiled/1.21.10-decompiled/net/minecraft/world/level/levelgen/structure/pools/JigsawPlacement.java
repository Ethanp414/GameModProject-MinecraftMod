...
public class JigsawPlacement {
   ...
   public static Optional<Structure.GenerationStub> addPieces(
      ...
      LevelHeightAccessor $$14 = $$0.heightAccessor();
      ...
      StructureTemplatePool $$18 = (StructureTemplatePool)$$1.unwrapKey().flatMap($$2x -> $$16.getOptional($$8.lookup($$2x))).orElse($$1.value());
      ...
      if ($$19 == EmptyPoolElement.INSTANCE) {
         ...
         BlockPos $$22;
         if ($$2.isPresent()) {
            ResourceLocation $$20 = (ResourceLocation)$$2.get();
            Optional<BlockPos> $$21 = getRandomNamedJigsaw($$19, $$20, $$4, $$17, $$13, $$15);
            if ($$21.isEmpty()) {
               LOGGER.error(
                  "No starting jigsaw {} found in start pool {}", $$20, $$1.unwrapKey().map($$0x -> $$0x.location().toString()).orElse("<unregistered>")
               );
               return Optional.empty();
            }
         ...
         int $$30 = $$6.isEmpty() ? $$25.getY() : $$4.getY() + $$12.getFirstFreeHeight($$28, $$29, (Heightmap.Types)$$6.get(), $$14, $$0.randomState());
         int $$31 = $$27.minY() + $$26.getGroundLevelDelta();
         $$26.move(0, $$30 - $$31, 0);
         if (isStartTooCloseToWorldHeightLimits($$14, $$9, $$26.getBoundingBox())) {
            LOGGER.debug("Center piece {} with bounding box {} does not fit dimension padding {}", $$19, $$26.getBoundingBox(), $$9);
            return Optional.empty();
         } else {
            int $$32 = $$30 + $$24.getY();
            return Optional.of(
               new Structure.GenerationStub(
                  ...
                  $$17x -> {
                     List<PoolElementStructurePiece> $$18xx = Lists.<PoolElementStructurePiece>newArrayList();
                     $$18xx.add($$26);
                     if ($$3 > 0) {
                        AABB $$19xx = new AABB(
                           (double)($$28 - $$7.horizontal()),
                           (double)Math.max($$32 - $$7.vertical(), $$14.getMinY() + $$9.bottom()),
                           (double)($$29 - $$7.horizontal()),
                           (double)($$28 + $$7.horizontal() + 1),
                           (double)Math.min($$32 + $$7.vertical() + 1, $$14.getMaxY() + 1 - $$9.top()),
                           (double)($$29 + $$7.horizontal() + 1)
                        );
                        VoxelShape $$20 = Shapes.join(Shapes.create($$19xx), Shapes.create(AABB.of($$27)), BooleanOp.ONLY_FIRST);
                        addPieces($$0.randomState(), $$3, $$5, $$12, $$13, $$14, $$15, $$16, $$26, $$18xx, $$20, $$8, $$10);
                        $$18xx.forEach($$17x::addPiece);
                     }
                  }
               )
            );
   ...
   private static Optional<BlockPos> getRandomNamedJigsaw(
      StructurePoolElement $$0, ResourceLocation $$1, BlockPos $$2, Rotation $$3, StructureTemplateManager $$4, WorldgenRandom $$5
   ) {
   ...
   public static boolean generateJigsaw(ServerLevel $$0, Holder<StructureTemplatePool> $$1, ResourceLocation $$2, int $$3, BlockPos $$4, boolean $$5) {
      ...
      StructureManager $$8 = $$0.structureManager();
      RandomSource $$9 = $$0.getRandom();
      Structure.GenerationContext $$10 = new Structure.GenerationContext(
         $$0.registryAccess(), $$6, $$6.getBiomeSource(), $$0.getChunkSource().randomState(), $$7, $$0.getSeed(), new ChunkPos($$4), $$0, $$0x -> true
      );
   ...
   static final class Placer {
      ...
      void tryPlacingChildren(
         ...
         for(StructureTemplate.JigsawBlockInfo $$16 : $$8.getShuffledJigsawBlocks(this.structureTemplateManager, $$9, $$10, this.random)) {
            ...
            if ($$24.isEmpty()) {
               JigsawPlacement.LOGGER.warn("Empty or non-existent pool: {}", $$23.location());
            } else {
               Holder<StructureTemplatePool> $$25 = (Holder)$$24.get();
               if ($$25.value().size() == 0 && !$$25.is(Pools.EMPTY)) {
                  JigsawPlacement.LOGGER.warn("Empty or non-existent pool: {}", $$23.location());
               } else {
                  Holder<StructureTemplatePool> $$26 = $$25.value().getFallback();
                  if ($$26.value().size() == 0 && !$$26.is(Pools.EMPTY)) {
                     JigsawPlacement.LOGGER
                        .warn("Empty or non-existent fallback pool: {}", $$26.unwrapKey().map($$0x -> $$0x.location().toString()).orElse("<unregistered>"));
                  } else {
                     ...
                     List<StructurePoolElement> $$30 = Lists.<StructurePoolElement>newArrayList();
                     if ($$2 != this.maxDepth) {
                        $$30.addAll($$25.value().getShuffledTemplates(this.random));
                     }
                        ...
                        for(Rotation $$33 : Rotation.getShuffled(this.random)) {
                           ...
                           if ($$3 && $$35.getYSpan() <= 16) {
                              $$37 = $$34.stream().mapToInt($$2x -> {
                                 StructureTemplate.StructureBlockInfo $$3xx = $$2x.info();
                                 if (!$$35.isInside($$3xx.pos().relative(JigsawBlock.getFrontFacing($$3xx.state())))) {
                                    return 0;
                                 } else {
                                    ResourceKey<StructureTemplatePool> $$4xx = $$6.lookup($$2x.pool());
                                    Optional<? extends Holder<StructureTemplatePool>> $$5xx = this.pools.get($$4xx);
                                    Optional<Holder<StructureTemplatePool>> $$6xx = $$5xx.map($$0xx -> ((StructureTemplatePool)$$0xx.value()).getFallback());
                                    int $$7xx = $$5xx.map($$0xx -> ((StructureTemplatePool)$$0xx.value()).getMaxSize(this.structureTemplateManager)).orElse(0);
                                    int $$8xx = $$6xx.map($$0xx -> ((StructureTemplatePool)$$0xx.value()).getMaxSize(this.structureTemplateManager)).orElse(0);
                                    return Math.max($$7xx, $$8xx);
                                 }
                              }).max().orElse(0);
                           } else {
                              $$37 = 0;
                           ...
                           for(StructureTemplate.JigsawBlockInfo $$38 : $$34) {
                              if (JigsawBlock.canAttach($$16, $$38)) {
                                 ...
                                 if (!Shapes.joinIsNotEmpty($$28.getValue(), Shapes.create(AABB.of($$50).deflate(0.25)), BooleanOp.ONLY_SECOND)) {
                                    $$28.setValue(Shapes.joinUnoptimized($$28.getValue(), Shapes.create(AABB.of($$50)), BooleanOp.ONLY_FIRST));
