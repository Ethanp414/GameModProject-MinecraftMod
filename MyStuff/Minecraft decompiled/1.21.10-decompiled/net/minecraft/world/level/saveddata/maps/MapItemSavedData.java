...
public class MapItemSavedData extends SavedData {
   ...
   public static final Codec<MapItemSavedData> CODEC = RecordCodecBuilder.create(
      $$0 -> $$0.group(
               Level.RESOURCE_KEY_CODEC.fieldOf("dimension").forGetter($$0x -> $$0x.dimension),
               Codec.INT.fieldOf("xCenter").forGetter($$0x -> $$0x.centerX),
               Codec.INT.fieldOf("zCenter").forGetter($$0x -> $$0x.centerZ),
               Codec.BYTE.optionalFieldOf("scale", Byte.valueOf((byte)0)).forGetter($$0x -> $$0x.scale),
               Codec.BYTE_BUFFER.fieldOf("colors").forGetter($$0x -> ByteBuffer.wrap($$0x.colors)),
               Codec.BOOL.optionalFieldOf("trackingPosition", Boolean.valueOf(true)).forGetter($$0x -> $$0x.trackingPosition),
               Codec.BOOL.optionalFieldOf("unlimitedTracking", Boolean.valueOf(false)).forGetter($$0x -> $$0x.unlimitedTracking),
               Codec.BOOL.optionalFieldOf("locked", Boolean.valueOf(false)).forGetter($$0x -> $$0x.locked),
   ...
   private MapItemSavedData(
      ResourceKey<Level> $$0, int $$1, int $$2, byte $$3, ByteBuffer $$4, boolean $$5, boolean $$6, boolean $$7, List<MapBanner> $$8, List<MapFrame> $$9
   ) {
      ...
      for(MapBanner $$10 : $$8) {
         ...
         this.addDecoration(
            $$10.getDecoration(), null, $$10.getId(), (double)$$10.pos().getX(), (double)$$10.pos().getZ(), 180.0, (Component)$$10.name().orElse(null)
         );
      }
      ...
      for(MapFrame $$11 : $$9) {
         ...
         this.addDecoration(
            MapDecorationTypes.FRAME, null, getFrameKey($$11.entityId()), (double)$$11.pos().getX(), (double)$$11.pos().getZ(), (double)$$11.rotation(), null
         );
      }
   }
   ...
   public void tickCarriedBy(Player $$0, ItemStack $$1) {
      ...
      for(int $$4 = 0; $$4 < this.carriedBy.size(); ++$$4) {
         MapItemSavedData.HoldingPlayer $$5 = (MapItemSavedData.HoldingPlayer)this.carriedBy.get($$4);
         Player $$6 = $$5.player;
         ...
         if (!$$6.isRemoved() && ($$6.getInventory().contains($$3) || $$1.isFramed())) {
            if (!$$1.isFramed() && $$6.level().dimension() == this.dimension && this.trackingPosition) {
               this.addDecoration(MapDecorationTypes.PLAYER, $$6.level(), $$7, $$6.getX(), $$6.getZ(), (double)$$6.getYRot(), null);
            }
         } else {
      ...
      if ($$1.isFramed() && this.trackingPosition) {
         ...
         BlockPos $$9 = $$8.getPos();
         MapFrame $$10 = (MapFrame)this.frameMarkers.get(MapFrame.frameId($$9));
         ...
         this.addDecoration(
            ...
            (double)$$9.getX(),
            (double)$$9.getZ(),
            (double)($$8.getDirection().get2DDataValue() * 90),
            null
         );
         MapFrame $$12 = (MapFrame)this.frameMarkers.put($$11.getId(), $$11);
         if (!$$11.equals($$12)) {
            this.setDirty();
         }
      }

      MapDecorations $$13 = $$1.getOrDefault(DataComponents.MAP_DECORATIONS, MapDecorations.EMPTY);
      if (!this.decorations.keySet().containsAll($$13.decorations().keySet())) {
         $$13.decorations().forEach(($$1x, $$2) -> {
            if (!this.decorations.containsKey($$1x)) {
               this.addDecoration($$2.type(), $$0.level(), $$1x, $$2.x(), $$2.z(), (double)$$2.rotation(), null);
            }
         });
      }
   }
   ...
   private void removeDecoration(String $$0) {
      MapDecoration $$1 = (MapDecoration)this.decorations.remove($$0);
   ...
   public static void addTargetDecoration(ItemStack $$0, BlockPos $$1, String $$2, Holder<MapDecorationType> $$3) {
      MapDecorations.Entry $$4 = new MapDecorations.Entry($$3, (double)$$1.getX(), (double)$$1.getZ(), 180.0F);
   ...
   private void addDecoration(
      Holder<MapDecorationType> $$0, @Nullable LevelAccessor $$1, String $$2, double $$3, double $$4, double $$5, @Nullable Component $$6
   ) {
      int $$7 = 1 << this.scale;
      ...
      if ($$10 == null) {
         ...
         MapDecoration $$12 = (MapDecoration)this.decorations.put($$2, $$11);
         if (!$$11.equals($$12)) {
   ...
   private MapItemSavedData.MapDecorationLocation calculateDecorationLocationAndType(
      Holder<MapDecorationType> $$0, @Nullable LevelAccessor $$1, double $$2, float $$3, float $$4
   ) {
      ...
      if ($$0.is(MapDecorationTypes.PLAYER)) {
         ...
         return $$7 == null ? null : new MapItemSavedData.MapDecorationLocation($$7.getFirst(), $$5, $$6, $$7.getSecond());
      } else {
   ...
   public Packet<?> getUpdatePacket(MapId $$0, Player $$1) {
      MapItemSavedData.HoldingPlayer $$2 = (MapItemSavedData.HoldingPlayer)this.carriedByPlayers.get($$1);
   ...
   public MapItemSavedData.HoldingPlayer getHoldingPlayer(Player $$0) {
      MapItemSavedData.HoldingPlayer $$1 = (MapItemSavedData.HoldingPlayer)this.carriedByPlayers.get($$0);
      if ($$1 == null) {
   ...
   public boolean toggleBanner(LevelAccessor $$0, BlockPos $$1) {
      ...
      if ($$5 >= -63.0 && $$6 >= -63.0 && $$5 <= 63.0 && $$6 <= 63.0) {
         ...
         if (!this.isTrackedCountOverLimit(256)) {
            ...
            this.addDecoration($$8.getDecoration(), $$0, $$8.getId(), $$2, $$3, 180.0, (Component)$$8.name().orElse(null));
            this.setDirty();
            return true;
         }
      }
   ...
   public static record MapPatch(int startX, int startY, int width, int height, byte[] mapColors) {
      ...
      private static void write(ByteBuf $$0, Optional<MapItemSavedData.MapPatch> $$1) {
         if ($$1.isPresent()) {
            MapItemSavedData.MapPatch $$2 = (MapItemSavedData.MapPatch)$$1.get();
            $$0.writeByte($$2.width);
