...
public class Climate {
   ...
   public static class ParameterList<T> {
      ...
      public T findValueBruteForce(Climate.TargetPoint $$0) {
         ...
         Pair<Climate.ParameterPoint, T> $$2 = (Pair)$$1.next();
         long $$3 = ((Climate.ParameterPoint)$$2.getFirst()).fitness($$0);
         T $$4 = $$2.getSecond();

         while($$1.hasNext()) {
            Pair<Climate.ParameterPoint, T> $$5 = (Pair)$$1.next();
   ...
   protected static final class RTree<T> {
      ...
      public static <T> Climate.RTree<T> create(List<Pair<Climate.ParameterPoint, T>> $$0) {
         if ($$0.isEmpty()) {
            ...
            int $$1 = ((Climate.ParameterPoint)((Pair)$$0.get(0)).getFirst()).parameterSpace().size();
            if ($$1 != 7) {
               throw new IllegalStateException("Expecting parameter space to be 7, got " + $$1);
            } else {
               List<Climate.RTree.Leaf<T>> $$2 = (List)$$0.stream()
      ...
      private static <T> Climate.RTree.Node<T> build(int $$0, List<? extends Climate.RTree.Node<T>> $$1) {
         if ($$1.isEmpty()) {
            ...
            $$1.sort(Comparator.comparingLong($$1x -> {
               ...
               for(int $$3xx = 0; $$3xx < $$0; ++$$3xx) {
                  Climate.Parameter $$4xx = $$1x.parameterSpace[$$3xx];
                  $$2xx += Math.abs(($$4xx.min() + $$4xx.max()) / 2L);
               }
            ...
            return new Climate.RTree.SubTree<>(
               (List<? extends Climate.RTree.Node<T>>)$$4.stream().map($$1x -> build($$0, Arrays.asList($$1x.children))).collect(Collectors.toList())
            );
         }
      }
      ...
      static <T> List<Climate.Parameter> buildParameterSpace(List<? extends Climate.RTree.Node<T>> $$0) {
         if ($$0.isEmpty()) {
            ...
            for(Climate.RTree.Node<T> $$4 : $$0) {
               for(int $$5 = 0; $$5 < 7; ++$$5) {
                  $$2.set($$5, $$4.parameterSpace[$$5].span((Climate.Parameter)$$2.get($$5)));
               }
            }
      ...
      public T search(Climate.TargetPoint $$0, Climate.DistanceMetric<T> $$1) {
         ...
         Climate.RTree.Leaf<T> $$3 = this.root.search($$2, (Climate.RTree.Leaf<T>)this.lastResult.get(), $$1);
      ...
      abstract static class Node<T> {
         ...
         protected Node(List<Climate.Parameter> $$0) {
            this.parameterSpace = (Climate.Parameter[])$$0.toArray(new Climate.Parameter[0]);
         }
      ...
      static final class SubTree<T> extends Climate.RTree.Node<T> {
         ...
         protected SubTree(List<Climate.Parameter> $$0, List<? extends Climate.RTree.Node<T>> $$1) {
            super($$0);
            this.children = (Climate.RTree.Node[])$$1.toArray(new Climate.RTree.Node[0]);
         }
