...
public class DimensionDataStorage implements AutoCloseable {
   ...
   public <T extends SavedData> T computeIfAbsent(SavedDataType<T> $$0) {
      ...
      if ($$1 != null) {
         ...
         T $$2 = (T)$$0.constructor().apply(this.context);
         this.set($$0, $$2);
         return $$2;
      }
   }
   ...
   public <T extends SavedData> T get(SavedDataType<T> $$0) {
      Optional<SavedData> $$1 = (Optional)this.cache.get($$0);
      if ($$1 == null) {
         $$1 = Optional.ofNullable(this.<T>readSavedData($$0));
         this.cache.put($$0, $$1);
      }
   ...
   private <T extends SavedData> T readSavedData(SavedDataType<T> $$0) {
      try {
         ...
         if (Files.exists($$1, new LinkOption[0])) {
            ...
            return (T)((Codec)$$0.codec().apply(this.context))
               .parse($$3, $$2.get("data"))
               .resultOrPartial($$1 -> LOGGER.error("Failed to parse saved data for '{}': {}", $$0, $$1))
               .orElse(null);
         }
            ...
            this.pendingWriteFuture = this.pendingWriteFuture.thenCompose($$3 -> {
               ...
               for(List<Entry<SavedDataType<?>, CompoundTag>> $$6 : Iterables.partition($$0.entrySet(), $$5)) {
                  $$4.add(CompletableFuture.runAsync(() -> {
                     for(Entry<SavedDataType<?>, CompoundTag> $$1xxx : $$6) {
                        this.tryWrite((SavedDataType<?>)$$1xxx.getKey(), (CompoundTag)$$1xxx.getValue());
                     }
                  }, Util.ioPool()));
               }

               return CompletableFuture.allOf((CompletableFuture[])$$4.toArray($$0xx -> new CompletableFuture[$$0xx]));
            });
         } else {
            this.pendingWriteFuture = this.pendingWriteFuture
               .thenCompose(
                  $$1x -> CompletableFuture.allOf(
                        (CompletableFuture[])$$0.entrySet()
                           .stream()
                           .map(
   ...
   private <T extends SavedData> CompoundTag encodeUnchecked(SavedDataType<T> $$0, SavedData $$1, RegistryOps<Tag> $$2) {
      Codec<T> $$3 = (Codec)$$0.codec().apply(this.context);
      CompoundTag $$4 = new CompoundTag();
      $$4.put("data", $$3.encodeStart($$2, (T)$$1).getOrThrow());
