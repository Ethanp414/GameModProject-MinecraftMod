...
public class BuiltInLootTables {
   ...
   private static Map<DyeColor, ResourceKey<LootTable>> makeDyeKeyMap(String $$0) {
      return Util.makeEnumMap(DyeColor.class, $$1 -> register($$0 + "/" + $$1.getName()));
   }

   private static ResourceKey<LootTable> register(String $$0) {
      return register(ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace($$0)));
   }
   ...
   private static ResourceKey<LootTable> register(ResourceKey<LootTable> $$0) {
      if (LOCATIONS.add($$0)) {
         ...
         throw new IllegalArgumentException($$0.location() + " is already a registered built-in loot table");
      }
   }
