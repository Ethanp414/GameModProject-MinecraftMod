...
public class IntRange {
   ...
   public static final Codec<IntRange> CODEC = Codec.either(Codec.INT, RECORD_CODEC).xmap($$0 -> $$0.map(IntRange::exact, Function.identity()), $$0 -> {
   ...
   public static IntRange exact(int $$0) {
      ConstantValue $$1 = ConstantValue.exactly((float)$$0);
      return new IntRange(Optional.of($$1), Optional.of($$1));
   }

   public static IntRange range(int $$0, int $$1) {
      return new IntRange(Optional.of(ConstantValue.exactly((float)$$0)), Optional.of(ConstantValue.exactly((float)$$1)));
   }

   public static IntRange lowerBound(int $$0) {
      return new IntRange(Optional.of(ConstantValue.exactly((float)$$0)), Optional.empty());
   }

   public static IntRange upperBound(int $$0) {
      return new IntRange(Optional.empty(), Optional.of(ConstantValue.exactly((float)$$0)));
   }
   ...
   private OptionalInt unpackExact() {
      if (Objects.equals(this.min, this.max)) {
         ...
         if (var2 instanceof ConstantValue $$0 && Math.floor((double)$$0.value()) == (double)$$0.value()) {
