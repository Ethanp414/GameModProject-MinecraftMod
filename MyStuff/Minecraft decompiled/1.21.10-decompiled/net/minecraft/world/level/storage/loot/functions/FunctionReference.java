...
public class FunctionReference extends LootItemConditionalFunction {
   ...
   public static final MapCodec<FunctionReference> CODEC = RecordCodecBuilder.mapCodec(
      $$0 -> commonFields($$0)
            .and(ResourceKey.codec(Registries.ITEM_MODIFIER).fieldOf("name").forGetter($$0x -> $$0x.name))
            .apply($$0, FunctionReference::new)
   );
   ...
   public void validate(ValidationContext $$0) {
      if (!$$0.allowsReferences()) {
         ...
         $$0.resolver()
            .get(this.name)
            .ifPresentOrElse(
               $$1 -> ((LootItemFunction)$$1.value()).validate($$0.enterElement(new ProblemReporter.ElementReferencePathElement(this.name), this.name)),
   ...
   protected ItemStack run(ItemStack $$0, LootContext $$1) {
      LootItemFunction $$2 = (LootItemFunction)$$1.getResolver().get(this.name).map(Holder::value).orElse(null);
      if ($$2 == null) {
