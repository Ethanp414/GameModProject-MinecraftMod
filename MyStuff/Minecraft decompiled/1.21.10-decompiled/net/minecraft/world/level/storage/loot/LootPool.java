...
public class LootPool {
   ...
   private void addRandomItem(Consumer<ItemStack> $$0, LootContext $$1) {
      ...
      List<LootPoolEntry> $$3 = Lists.<LootPoolEntry>newArrayList();
      MutableInt $$4 = new MutableInt();

      for(LootPoolEntryContainer $$5 : this.entries) {
         $$5.expand($$1, $$3x -> {
            int $$4xx = $$3x.getWeight($$1.getLuck());
            if ($$4xx > 0) {
               $$3.add($$3x);
               $$4.add($$4xx);
            }
   ...
   public void validate(ValidationContext $$0) {
      for(int $$1 = 0; $$1 < this.conditions.size(); ++$$1) {
         ((LootItemCondition)this.conditions.get($$1)).validate($$0.forChild(new ProblemReporter.IndexedFieldPathElement("conditions", $$1)));
      }

      for(int $$2 = 0; $$2 < this.functions.size(); ++$$2) {
         ((LootItemFunction)this.functions.get($$2)).validate($$0.forChild(new ProblemReporter.IndexedFieldPathElement("functions", $$2)));
      }

      for(int $$3 = 0; $$3 < this.entries.size(); ++$$3) {
         ((LootPoolEntryContainer)this.entries.get($$3)).validate($$0.forChild(new ProblemReporter.IndexedFieldPathElement("entries", $$3)));
      }
