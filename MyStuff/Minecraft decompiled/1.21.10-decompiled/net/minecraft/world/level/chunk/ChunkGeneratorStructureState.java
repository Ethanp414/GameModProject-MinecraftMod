...
public class ChunkGeneratorStructureState {
   ...
   public static ChunkGeneratorStructureState createForNormal(RandomState $$0, long $$1, BiomeSource $$2, HolderLookup<StructureSet> $$3) {
      List<Holder<StructureSet>> $$4 = (List)$$3.listElements()
   ...
   private static boolean hasBiomesForStructureSet(StructureSet $$0, BiomeSource $$1) {
      Stream<Holder<Biome>> $$2 = $$0.structures().stream().flatMap($$0x -> {
         Structure $$1xx = $$0x.structure().value();
         return $$1xx.biomes().stream();
      });
      ...
      this.possibleStructureSets().forEach($$1 -> {
         ...
         for(StructureSet.StructureSelectionEntry $$4 : $$2.structures()) {
            ...
            if ($$5.biomes().stream().anyMatch($$0::contains)) {
               ((List)this.placementsForStructure.computeIfAbsent($$5, $$0xx -> new ArrayList())).add($$2.placement());
               $$3 = true;
            }
         }
         ...
         if ($$3) {
            ...
            if ($$6 instanceof ConcentricRingsStructurePlacement $$7) {
               this.ringPositions.put($$7, this.generateRingPositions($$1, $$7));
            }
         }
      });
   }
   ...
   private CompletableFuture<List<ChunkPos>> generateRingPositions(Holder<StructureSet> $$0, ConcentricRingsStructurePlacement $$1) {
      if ($$1.count() == 0) {
         ...
         for(int $$12 = 0; $$12 < $$4; ++$$12) {
            ...
            $$5.add(
               CompletableFuture.supplyAsync(
                  () -> {
                     Pair<BlockPos, Holder<Biome>> $$4xx = this.biomeSource
                        .findBiomeHorizontal(
                     ...
                     if ($$4xx != null) {
                        BlockPos $$5xx = $$4xx.getFirst();
                        return new ChunkPos(SectionPos.blockToSectionCoord($$5xx.getX()), SectionPos.blockToSectionCoord($$5xx.getZ()));
                     } else {
         ...
         return Util.sequence($$5).thenApply($$2x -> {
            double $$3xx = (double)$$2.stop().elapsed(TimeUnit.MILLISECONDS) / 1000.0;
            LOGGER.debug("Calculation for {} took {}s", $$0, $$3xx);
            return $$2x;
         });
      }
   }
   ...
   public List<ChunkPos> getRingPositionsFor(ConcentricRingsStructurePlacement $$0) {
      ...
      CompletableFuture<List<ChunkPos>> $$1 = (CompletableFuture)this.ringPositions.get($$0);
      return $$1 != null ? (List)$$1.join() : null;
   }

   public List<StructurePlacement> getPlacementsForStructure(Holder<Structure> $$0) {
      this.ensureStructuresGenerated();
      return (List<StructurePlacement>)this.placementsForStructure.getOrDefault($$0.value(), List.of());
   }
