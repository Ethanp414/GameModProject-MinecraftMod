...
public class IOWorker implements ChunkScanAccess, AutoCloseable {
   ...
   private CompletableFuture<BitSet> createOldDataForRegion(int $$0, int $$1) {
      return CompletableFuture.supplyAsync(() -> {
         ...
         BitSet $$4 = new BitSet();
         ChunkPos.rangeClosed($$2, $$3).forEach($$1xx -> {
            CollectFields $$2xx = new CollectFields(new FieldSelector(IntTag.TYPE, "DataVersion"), new FieldSelector(CompoundTag.TYPE, "blending_data"));

            try {
               this.scanChunk($$1xx, $$2xx).join();
            } catch (Exception var7) {
               LOGGER.warn("Failed to scan chunk {}", $$1xx, var7);
               return;
            }

            Tag $$4xx = $$2xx.getResult();
            if ($$4xx instanceof CompoundTag $$5 && this.isOldChunk($$5)) {
   ...
   public CompletableFuture<Void> store(ChunkPos $$0, Supplier<CompoundTag> $$1) {
      return this.submitTask(() -> {
         ...
         IOWorker.PendingStore $$3 = (IOWorker.PendingStore)this.pendingWrites.computeIfAbsent($$0, $$1xx -> new IOWorker.PendingStore($$2));
         $$3.data = $$2;
         return $$3.result;
   ...
   public CompletableFuture<Optional<CompoundTag>> loadAsync(ChunkPos $$0) {
      return this.submitThrowingTask(() -> {
         IOWorker.PendingStore $$1 = (IOWorker.PendingStore)this.pendingWrites.get($$0);
         if ($$1 != null) {
   ...
   public CompletableFuture<Void> synchronize(boolean $$0) {
      CompletableFuture<Void> $$1 = this.submitTask(
            () -> CompletableFuture.allOf(
                  (CompletableFuture[])this.pendingWrites.values().stream().map($$0x -> $$0x.result).toArray($$0x -> new CompletableFuture[$$0x])
               )
         )
         .thenCompose(Function.identity());
   ...
   public CompletableFuture<Void> scanChunk(ChunkPos $$0, StreamTagVisitor $$1) {
      return this.submitThrowingTask(() -> {
         try {
            IOWorker.PendingStore $$2 = (IOWorker.PendingStore)this.pendingWrites.get($$0);
            if ($$2 != null) {
               if ($$2.data != null) {
   ...
   private void storePendingChunk() {
      Entry<ChunkPos, IOWorker.PendingStore> $$0 = this.pendingWrites.pollFirstEntry();
      if ($$0 != null) {
         this.runStore((ChunkPos)$$0.getKey(), (IOWorker.PendingStore)$$0.getValue());
         this.tellStorePending();
      }
   }
   ...
   private void runStore(ChunkPos $$0, IOWorker.PendingStore $$1) {
      try {
