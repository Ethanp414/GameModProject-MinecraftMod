...
public abstract class Strategy<T> {
   ...
   Strategy(IdMap<T> $$0, int $$1) {
      this.globalMap = $$0;
      this.globalPalette = new GlobalPalette<>($$0);
      this.globalPaletteBitsInMemory = minimumBitsRequiredForDistinctValues($$0.size());
      this.bitsPerAxis = $$1;
      this.entryCount = 1 << $$1 * 3;
   }

   public static <T> Strategy<T> createForBlockStates(IdMap<T> $$0) {
      return new Strategy<T>($$0, 4) {
         @Override
         public Configuration getConfigurationForBitCount(int $$0) {
            return (Configuration)(switch($$0) {
               ...
               default -> new Configuration.Global(this.globalPaletteBitsInMemory, $$0);
            });
         }
      };
   }

   public static <T> Strategy<T> createForBiomes(IdMap<T> $$0) {
      return new Strategy<T>($$0, 2) {
         @Override
         public Configuration getConfigurationForBitCount(int $$0) {
            return (Configuration)(switch($$0) {
               case 0 -> Strategy.ZERO_BITS;
   ...
   public int getIndex(int $$0, int $$1, int $$2) {
      return ($$1 << this.bitsPerAxis | $$2) << this.bitsPerAxis | $$0;
   }
   ...
   protected Configuration getConfigurationForPaletteSize(int $$0) {
      int $$1 = minimumBitsRequiredForDistinctValues($$0);
      return this.getConfigurationForBitCount($$1);
   }

   private static int minimumBitsRequiredForDistinctValues(int $$0) {
      return Mth.ceillog2($$0);
   }
}
