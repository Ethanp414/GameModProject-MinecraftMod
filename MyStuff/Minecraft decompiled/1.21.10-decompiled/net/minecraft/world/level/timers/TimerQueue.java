...
public class TimerQueue<T> {
   ...
   public TimerQueue(TimerCallbacks<T> $$0, Stream<? extends Dynamic<?>> $$1) {
      ...
      $$1.forEach($$0x -> {
         Tag $$1xx = $$0x.convert(NbtOps.INSTANCE).getValue();
         if ($$1xx instanceof CompoundTag $$2) {
            this.loadEvent($$2);
         } else {
            LOGGER.warn("Invalid format of events: {}", $$1xx);
         }
      });
   }
   ...
   public void tick(T $$0, long $$1) {
      while(true) {
         TimerQueue.Event<T> $$2 = (TimerQueue.Event)this.queue.peek();
   ...
   private void loadEvent(CompoundTag $$0) {
      TimerCallback<T> $$1 = (TimerCallback)$$0.read("Callback", this.callbacksRegistry.codec()).orElse(null);
      if ($$1 != null) {
