...
public class PiglinAi {
   ...
   protected static void initMemories(Piglin $$0, RandomSource $$1) {
      ...
      $$0.getBrain().setMemoryWithExpiry(MemoryModuleType.HUNTED_RECENTLY, true, (long)$$2);
   }
   ...
   private static void initIdleActivity(Brain<Piglin> $$0) {
      $$0.addActivity(
         ...
         ImmutableList.of(
            ...
            StartAttacking.<Piglin>create(($$0x, $$1) -> $$1.isAdult(), PiglinAi::findNearestValidAttackTarget),
            ...
            BehaviorBuilder.sequence(
               ...
               TriggerGate.triggerOneShuffled(
                  ImmutableList.<Pair<? extends Trigger<? super LivingEntity>, Integer>>builder()
                     ...
                     .add(Pair.of(BehaviorBuilder.triggerIf((Predicate<? super LivingEntity>)($$0x -> true)), 1))
                     .build()
               )
            ),
   ...
   private static void throwItems(Piglin $$0, List<ItemStack> $$1) {
      ...
      if ($$2.isPresent()) {
         throwItemsTowardPlayer($$0, (Player)$$2.get(), $$1);
      } else {
   ...
   private static List<ItemStack> getBarterResponseItems(Piglin $$0) {
      ...
      List<ItemStack> $$2 = $$1.getRandomItems(
         new LootParams.Builder((ServerLevel)$$0.level()).withParameter(LootContextParams.THIS_ENTITY, $$0).create(LootContextParamSets.PIGLIN_BARTER)
      );
      return $$2;
   }
   ...
   protected static boolean wantsToPickup(Piglin $$0, ItemStack $$1) {
      if ($$0.isBaby() && $$1.is(ItemTags.IGNORED_BY_PIGLIN_BABIES)) {
         ...
         } else if (!isLovedItem($$1)) {
            return $$0.canReplaceCurrentItem($$1);
         } else {
            return isNotHoldingLovedItemInOffHand($$0) && $$2;
         }
      }
   }
   ...
   private static boolean wantsToStopRiding(Piglin $$0, Entity $$1) {
      if (!($$1 instanceof Mob)) {
         ...
         Mob $$2 = (Mob)$$1;
         return !$$2.isBaby() || !$$2.isAlive() || wasHurtRecently($$0) || wasHurtRecently($$2) || $$2 instanceof Piglin && $$2.getVehicle() == null;
      }
   }

   private static boolean isNearestValidAttackTarget(ServerLevel $$0, Piglin $$1, LivingEntity $$2) {
   ...
   private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel $$0, Piglin $$1) {
      ...
      if (isNearZombified($$1)) {
         ...
         if ($$3.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight($$0, $$1, (LivingEntity)$$3.get())) {
            return $$3;
         } else {
            ...
            if ($$5.isPresent()) {
               ...
               return $$6.isPresent() && Sensor.isEntityAttackable($$0, $$1, (LivingEntity)$$6.get()) ? $$6 : Optional.empty();
            }
         }
      }
   }
   ...
   protected static void wasHurtBy(ServerLevel $$0, Piglin $$1, LivingEntity $$2) {
      if (!($$2 instanceof Piglin)) {
         ...
         } else if ($$2.getType() == EntityType.HOGLIN && hoglinsOutnumberPiglins($$1)) {
   ...
   private static boolean isNearAvoidTarget(Piglin $$0) {
      Brain<Piglin> $$1 = $$0.getBrain();
      return !$$1.hasMemoryValue(MemoryModuleType.AVOID_TARGET)
         ? false
         : ((LivingEntity)$$1.getMemory(MemoryModuleType.AVOID_TARGET).get()).closerThan($$0, 12.0);
   }

   protected static List<AbstractPiglin> getVisibleAdultPiglins(Piglin $$0) {
      return (List<AbstractPiglin>)$$0.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS).orElse(ImmutableList.of());
   }

   private static List<AbstractPiglin> getAdultPiglins(AbstractPiglin $$0) {
      return (List<AbstractPiglin>)$$0.getBrain().getMemory(MemoryModuleType.NEARBY_ADULT_PIGLINS).orElse(ImmutableList.of());
   }
   ...
   protected static void broadcastAngerTarget(ServerLevel $$0, AbstractPiglin $$1, LivingEntity $$2) {
      getAdultPiglins($$1).forEach($$2x -> {
         if ($$2.getType() != EntityType.HOGLIN || $$2x.canHunt() && ((Hoglin)$$2).canBeHunted()) {
   ...
   private static void setAngerTargetToNearestTargetablePlayerIfFound(ServerLevel $$0, AbstractPiglin $$1, LivingEntity $$2) {
      ...
      if ($$3.isPresent()) {
         setAngerTarget($$0, $$1, (LivingEntity)$$3.get());
      } else {
         setAngerTarget($$0, $$1, $$2);
      }
   }
   ...
   private static void setAngerTargetIfCloserThanCurrent(ServerLevel $$0, AbstractPiglin $$1, LivingEntity $$2) {
      Optional<LivingEntity> $$3 = getAngerTarget($$1);
   ...
   private static void retreatFromNearestTarget(Piglin $$0, LivingEntity $$1) {
      ...
      LivingEntity $$3 = BehaviorUtils.getNearestTarget($$0, $$2.getMemory(MemoryModuleType.AVOID_TARGET), $$1);
   ...
   private static void setAvoidTargetAndDontHuntForAWhile(Piglin $$0, LivingEntity $$1) {
      ...
      $$0.getBrain().eraseMemory(MemoryModuleType.WALK_TARGET);
      $$0.getBrain().setMemoryWithExpiry(MemoryModuleType.AVOID_TARGET, $$1, (long)RETREAT_DURATION.sample($$0.level().random));
      dontKillAnyMoreHoglinsForAWhile($$0);
   }

   protected static void dontKillAnyMoreHoglinsForAWhile(AbstractPiglin $$0) {
      $$0.getBrain().setMemoryWithExpiry(MemoryModuleType.HUNTED_RECENTLY, true, (long)TIME_BETWEEN_HUNTS.sample($$0.level().random));
   }
