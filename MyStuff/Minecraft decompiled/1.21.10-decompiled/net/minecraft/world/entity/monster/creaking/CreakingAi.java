...
public class CreakingAi {
   ...
   static void initIdleActivity(Brain<Creaking> $$0) {
      $$0.addActivity(
         Activity.IDLE,
         10,
         ImmutableList.of(
            StartAttacking.create(($$0x, $$1) -> $$1.isActive(), ($$0x, $$1) -> $$1.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)),
   ...
   static void initFightActivity(Creaking $$0, Brain<Creaking> $$1) {
      $$1.addActivityWithConditions(
         ...
         ImmutableList.of(
            ...
            StopAttackingIfTargetInvalid.create(($$1x, $$2) -> !isAttackTargetStillReachable($$0, $$2))
         ),
         ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT))
      );
   }

   private static boolean isAttackTargetStillReachable(Creaking $$0, LivingEntity $$1) {
      Optional<List<Player>> $$2 = $$0.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYERS);
      return $$2.map($$1x -> {
         if ($$1 instanceof Player $$2xx && $$1x.contains($$2xx)) {
            return true;
         }
