...
public class Shoot extends Behavior<Breeze> {
   ...
   protected void start(ServerLevel $$0, Breeze $$1, long $$2) {
      $$1.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent($$1x -> $$1.setPose(Pose.SHOOTING));
      $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.BREEZE_SHOOT_CHARGING, Unit.INSTANCE, (long)SHOOT_INITIAL_DELAY_TICKS);
   ...
   protected void stop(ServerLevel $$0, Breeze $$1, long $$2) {
      ...
      $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.BREEZE_SHOOT_COOLDOWN, Unit.INSTANCE, (long)SHOOT_COOLDOWN_TICKS);
      $$1.getBrain().eraseMemory(MemoryModuleType.BREEZE_SHOOT);
   }

   protected void tick(ServerLevel $$0, Breeze $$1, long $$2) {
      Brain<Breeze> $$3 = $$1.getBrain();
      LivingEntity $$4 = (LivingEntity)$$3.getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
      if ($$4 != null) {
         ...
         if (!$$3.getMemory(MemoryModuleType.BREEZE_SHOOT_CHARGING).isPresent() && !$$3.getMemory(MemoryModuleType.BREEZE_SHOOT_RECOVERING).isPresent()) {
            $$3.setMemoryWithExpiry(MemoryModuleType.BREEZE_SHOOT_RECOVERING, Unit.INSTANCE, (long)SHOOT_RECOVER_DELAY_TICKS);
            double $$5 = $$4.getX() - $$1.getX();
            double $$6 = $$4.getY($$4.isPassenger() ? 0.8 : 0.3) - $$1.getFiringYPosition();
            double $$7 = $$4.getZ() - $$1.getZ();
            Projectile.spawnProjectileUsingShoot(
               new BreezeWindCharge($$1, $$0), $$0, ItemStack.EMPTY, $$5, $$6, $$7, 0.7F, (float)(5 - $$0.getDifficulty().getId() * 4)
            );
