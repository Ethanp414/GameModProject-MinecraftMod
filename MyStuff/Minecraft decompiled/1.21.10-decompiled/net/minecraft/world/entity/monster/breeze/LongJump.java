...
public class LongJump extends Behavior<Breeze> {
   ...
   public static boolean canRun(ServerLevel $$0, Breeze $$1) {
      if (!$$1.onGround() && !$$1.isInWater()) {
         ...
         LivingEntity $$2 = (LivingEntity)$$1.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
         if ($$2 == null) {
            return false;
   ...
   protected void start(ServerLevel $$0, Breeze $$1, long $$2) {
      if ($$1.getBrain().checkMemory(MemoryModuleType.BREEZE_JUMP_INHALING, MemoryStatus.VALUE_ABSENT)) {
         $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.BREEZE_JUMP_INHALING, Unit.INSTANCE, (long)INHALING_DURATION_TICKS);
      }

      $$1.setPose(Pose.INHALING);
      $$0.playSound(null, $$1, SoundEvents.BREEZE_CHARGE, SoundSource.HOSTILE, 1.0F, 1.0F);
      $$1.getBrain().getMemory(MemoryModuleType.BREEZE_JUMP_TARGET).ifPresent($$1x -> $$1.lookAt(EntityAnchorArgument.Anchor.EYES, $$1x.getCenter()));
   }
   ...
   private static BlockPos snapToSurface(LivingEntity $$0, Vec3 $$1) {
      ...
      if ($$3.getType() == HitResult.Type.BLOCK) {
         ...
         return $$5.getType() == HitResult.Type.BLOCK ? BlockPos.containing($$5.getLocation()).above() : null;
      }
   }
      ...
      for(int $$4 : Util.shuffledCopy(ALLOWED_ANGLES, $$1)) {
         ...
         if ($$6.isPresent()) {
            if ($$0.hasEffect(MobEffects.JUMP_BOOST)) {
               double $$7 = ((Vec3)$$6.get()).normalize().y * (double)$$0.getJumpBoostPower();
