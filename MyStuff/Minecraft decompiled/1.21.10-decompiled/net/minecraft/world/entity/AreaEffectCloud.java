...
public class AreaEffectCloud extends Entity implements TraceableEntity {
   ...
   public void setRadius(float $$0) {
      if (!this.level().isClientSide()) {
         this.getEntityData().set(DATA_RADIUS, Mth.clamp($$0, 0.0F, 32.0F));
      }
   }
      ...
      if (!$$0 || !this.random.nextBoolean()) {
         ...
         for(int $$7 = 0; $$7 < $$3; ++$$7) {
            ...
            if ($$2.getType() == ParticleTypes.ENTITY_EFFECT) {
               ...
               this.level()
                  .addAlwaysVisibleParticle($$2, $$10, $$11, $$12, (0.5 - this.random.nextDouble()) * 0.15, 0.01F, (0.5 - this.random.nextDouble()) * 0.15);
            }
         }
      }
   }

   private void serverTick(ServerLevel $$0) {
      if (this.duration != -1 && this.tickCount - this.waitTime >= this.duration) {
         this.discard();
      } else {
         boolean $$1 = this.isWaiting();
         boolean $$2 = this.tickCount < this.waitTime;
         if ($$1 != $$2) {
            this.setWaiting($$2);
         }
         ...
         if (!$$2) {
            ...
            if (this.tickCount % 5 == 0) {
               this.victims.entrySet().removeIf($$0x -> this.tickCount >= $$0x.getValue());
               if (!this.potionContents.hasEffects()) {
                  ...
                  if (!$$5.isEmpty()) {
                     for(LivingEntity $$6 : $$5) {
                        if (!this.victims.containsKey($$6) && $$6.isAffectedByPotions() && !$$4.stream().noneMatch($$6::canBeAffected)) {
                           double $$7 = $$6.getX() - this.getX();
                           double $$8 = $$6.getZ() - this.getZ();
                           double $$9 = $$7 * $$7 + $$8 * $$8;
                           if ($$9 <= (double)($$3 * $$3)) {
                              ...
                              for(MobEffectInstance $$10 : $$4) {
                                 if ($$10.getEffect().value().isInstantenous()) {
                                    $$10.getEffect().value().applyInstantenousEffect($$0, this, this.getOwner(), $$6, $$10.getAmplifier(), 0.5);
                                 } else {
   ...
   public void setOwner(@Nullable LivingEntity $$0) {
      this.owner = EntityReference.of($$0);
   }
   ...
   protected void readAdditionalSaveData(ValueInput $$0) {
      ...
      this.setCustomParticle((ParticleOptions)$$0.read("custom_particle", ParticleTypes.CODEC).orElse(null));
      this.setPotionContents((PotionContents)$$0.read("potion_contents", PotionContents.CODEC).orElse(PotionContents.EMPTY));
   ...
   public <T> T get(DataComponentType<? extends T> $$0) {
      if ($$0 == DataComponents.POTION_CONTENTS) {
         return castComponentValue($$0, this.potionContents);
      } else {
         return (T)($$0 == DataComponents.POTION_DURATION_SCALE ? castComponentValue($$0, Float.valueOf(this.potionDurationScale)) : super.get($$0));
      }
   }
   ...
   protected <T> boolean applyImplicitComponent(DataComponentType<T> $$0, T $$1) {
      if ($$0 == DataComponents.POTION_CONTENTS) {
         this.setPotionContents(castComponentValue(DataComponents.POTION_CONTENTS, $$1));
         return true;
      } else if ($$0 == DataComponents.POTION_DURATION_SCALE) {
         this.setPotionDurationScale(castComponentValue(DataComponents.POTION_DURATION_SCALE, $$1));
         return true;
      } else {
