...
public class Schedule {
   ...
   protected Timeline getTimelineFor(Activity $$0) {
      return (Timeline)this.timelines.get($$0);
   }

   protected List<Timeline> getAllTimelinesExceptFor(Activity $$0) {
      return (List<Timeline>)this.timelines.entrySet().stream().filter($$1 -> $$1.getKey() != $$0).map(Entry::getValue).collect(Collectors.toList());
   }
   ...
   public Activity getActivityAt(int $$0) {
      return (Activity)this.timelines
         .entrySet()
         .stream()
         .max(Comparator.comparingDouble($$1 -> (double)((Timeline)$$1.getValue()).getValueAt($$0)))
         .map(Entry::getKey)
