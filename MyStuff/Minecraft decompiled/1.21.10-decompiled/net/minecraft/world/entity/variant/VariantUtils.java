...
public class VariantUtils {
   ...
   public static <T> void writeVariant(ValueOutput $$0, Holder<T> $$1) {
      $$1.unwrapKey().ifPresent($$1x -> $$0.store("variant", ResourceLocation.CODEC, $$1x.location()));
   }

   public static <T> Optional<Holder<T>> readVariant(ValueInput $$0, ResourceKey<? extends Registry<T>> $$1) {
      return $$0.read("variant", ResourceLocation.CODEC).map($$1x -> ResourceKey.create($$1, $$1x)).flatMap($$0.lookup()::get);
   }

   public static <T extends PriorityProvider<SpawnContext, ?>> Optional<Holder.Reference<T>> selectVariantToSpawn(
      SpawnContext $$0, ResourceKey<Registry<T>> $$1
   ) {
      ServerLevelAccessor $$2 = $$0.level();
      Stream<Holder.Reference<T>> $$3 = $$2.registryAccess().lookupOrThrow($$1).listElements();
