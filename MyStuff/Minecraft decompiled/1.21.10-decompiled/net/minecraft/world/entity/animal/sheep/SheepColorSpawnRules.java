...
public class SheepColorSpawnRules {
   ...
   private static SheepColorSpawnRules.SheepColorProvider commonColors(DyeColor $$0) {
   ...
   public static DyeColor getSheepColor(Holder<Biome> $$0, RandomSource $$1) {
      SheepColorSpawnRules.SheepColorSpawnConfiguration $$2 = getSheepColorConfiguration($$0);
      return $$2.colors().get($$1);
   }

   private static SheepColorSpawnRules.SheepColorSpawnConfiguration getSheepColorConfiguration(Holder<Biome> $$0) {
      if ($$0.is(BiomeTags.SPAWNS_WARM_VARIANT_FARM_ANIMALS)) {
         return WARM_SPAWN_CONFIGURATION;
      } else {
         return $$0.is(BiomeTags.SPAWNS_COLD_VARIANT_FARM_ANIMALS) ? COLD_SPAWN_CONFIGURATION : TEMPERATE_SPAWN_CONFIGURATION;
      }
   }

   private static SheepColorSpawnRules.SheepColorProvider weighted(WeightedList<SheepColorSpawnRules.SheepColorProvider> $$0) {
      if ($$0.isEmpty()) {
         throw new IllegalArgumentException("List must be non-empty");
      } else {
         return $$1 -> $$0.getRandomOrThrow($$1).get($$1);
      }
   }

   private static SheepColorSpawnRules.SheepColorProvider single(DyeColor $$0) {
      return $$1 -> $$0;
   }
