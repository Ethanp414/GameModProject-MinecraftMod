...
public class GossipContainer {
   ...
   public Map<UUID, Object2IntMap<GossipType>> getGossipEntries() {
      ...
      this.gossips.keySet().forEach($$1 -> {
         GossipContainer.EntityGossips $$2 = (GossipContainer.EntityGossips)this.gossips.get($$1);
         $$0.put($$1, $$2.entries);
      });
      return $$0;
   }
   ...
   public void decay() {
      ...
      while($$0.hasNext()) {
         GossipContainer.EntityGossips $$1 = (GossipContainer.EntityGossips)$$0.next();
         $$1.decay();
         if ($$1.isEmpty()) {
            $$0.remove();
         }
   ...
   private Stream<GossipContainer.GossipEntry> unpack() {
      return this.gossips.entrySet().stream().flatMap($$0 -> ((GossipContainer.EntityGossips)$$0.getValue()).unpack((UUID)$$0.getKey()));
   }
   ...
   private Collection<GossipContainer.GossipEntry> selectGossipsForTransfer(RandomSource $$0, int $$1) {
      ...
      if ($$2.isEmpty()) {
         ...
         for(int $$5 = 0; $$5 < $$2.size(); ++$$5) {
            GossipContainer.GossipEntry $$6 = (GossipContainer.GossipEntry)$$2.get($$5);
         ...
         for(int $$8 = 0; $$8 < $$1; ++$$8) {
            ...
            $$7.add((GossipContainer.GossipEntry)$$2.get($$10 < 0 ? -$$10 - 1 : $$10));
         }
   ...
   private GossipContainer.EntityGossips getOrCreate(UUID $$0) {
      return (GossipContainer.EntityGossips)this.gossips.computeIfAbsent($$0, $$0x -> new GossipContainer.EntityGossips());
   }
   ...
   public void transferFrom(GossipContainer $$0, RandomSource $$1, int $$2) {
      ...
      $$3.forEach($$0x -> {
         int $$1xx = $$0x.value - $$0x.type.decayPerTransfer;
         if ($$1xx >= 2) {
            this.getOrCreate($$0x.target).entries.mergeInt($$0x.type, $$1xx, GossipContainer::mergeValuesForTransfer);
         }
      });
   }

   public int getReputation(UUID $$0, Predicate<GossipType> $$1) {
      GossipContainer.EntityGossips $$2 = (GossipContainer.EntityGossips)this.gossips.get($$0);
      return $$2 != null ? $$2.weightedValue($$1) : 0;
   }

   public long getCountForType(GossipType $$0, DoublePredicate $$1) {
      return this.gossips.values().stream().filter($$2 -> $$1.test((double)($$2.entries.getOrDefault($$0, 0) * $$0.weight))).count();
   }

   public void add(UUID $$0, GossipType $$1, int $$2) {
      GossipContainer.EntityGossips $$3 = this.getOrCreate($$0);
      $$3.entries.mergeInt($$1, $$2, (IntBinaryOperator)(($$1x, $$2x) -> this.mergeValuesForAddition($$1, $$1x, $$2x)));
   ...
   public void remove(GossipType $$0) {
      ...
      while($$1.hasNext()) {
         GossipContainer.EntityGossips $$2 = (GossipContainer.EntityGossips)$$1.next();
         $$2.remove($$0);
         if ($$2.isEmpty()) {
            $$1.remove();
         }
   ...
   static class EntityGossips {
      ...
      public void decay() {
         ...
         while($$0.hasNext()) {
            Entry<GossipType> $$1 = (Entry)$$0.next();
