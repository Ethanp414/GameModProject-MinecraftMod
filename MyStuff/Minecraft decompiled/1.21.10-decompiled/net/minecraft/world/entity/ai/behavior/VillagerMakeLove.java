...
public class VillagerMakeLove extends Behavior<Villager> {
   ...
   private void tryToGiveBirth(ServerLevel $$0, Villager $$1, Villager $$2) {
      ...
      if ($$3.isEmpty()) {
         ...
         if ($$4.isPresent()) {
            ...
            $$0.debugSynchronizers().updatePoi((BlockPos)$$3.get());
         }
      }
   }
   ...
   private boolean isBreedingPossible(Villager $$0) {
      ...
      Optional<AgeableMob> $$2 = $$1.getMemory(MemoryModuleType.BREED_TARGET).filter($$0x -> $$0x.getType() == EntityType.VILLAGER);
      if ($$2.isEmpty()) {
         return false;
      } else {
         return BehaviorUtils.targetIsValid($$1, MemoryModuleType.BREED_TARGET, EntityType.VILLAGER) && $$0.canBreed() && ((AgeableMob)$$2.get()).canBreed();
      }
   }
