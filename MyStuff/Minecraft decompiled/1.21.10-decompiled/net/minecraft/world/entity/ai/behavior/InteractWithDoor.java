...
public class InteractWithDoor {
   ...
   public static BehaviorControl<LivingEntity> create() {
      ...
      return BehaviorBuilder.create(
         $$2 -> $$2.<MemoryAccessor, MemoryAccessor, MemoryAccessor>group(
                  $$2.present(MemoryModuleType.PATH),
                  $$2.registered(MemoryModuleType.DOORS_TO_CLOSE),
                  $$2.registered(MemoryModuleType.NEAREST_LIVING_ENTITIES)
               )
               .apply($$2, ($$3, $$4, $$5) -> ($$6, $$7, $$8) -> {
                     Path $$9 = $$2.get($$3);
   ...
   public static void closeDoorsThatIHaveOpenedOrPassedThrough(
      ServerLevel $$0, LivingEntity $$1, @Nullable Node $$2, @Nullable Node $$3, Set<GlobalPos> $$4, Optional<List<LivingEntity>> $$5
   ) {
   ...
   private static boolean isMobComingThroughDoor(Brain<?> $$0, BlockPos $$1) {
      if (!$$0.hasMemoryValue(MemoryModuleType.PATH)) {
         ...
         Path $$2 = (Path)$$0.getMemory(MemoryModuleType.PATH).get();
         if ($$2.isDone()) {
            return false;
         } else {
   ...
   private static Optional<Set<GlobalPos>> rememberDoorToClose(
      ...
      return Optional.of((Set)$$1.map($$1x -> {
         $$1x.add($$4);
         return $$1x;
      }).orElseGet(() -> {
         Set<GlobalPos> $$2xx = Sets.newHashSet($$4);
         $$0.set($$2xx);
         return $$2xx;
      }));
   }
