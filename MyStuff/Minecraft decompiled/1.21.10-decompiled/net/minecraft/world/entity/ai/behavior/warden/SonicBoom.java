...
public class SonicBoom extends Behavior<Warden> {
   ...
   protected void start(ServerLevel $$0, Warden $$1, long $$2) {
      $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.ATTACK_COOLING_DOWN, true, (long)DURATION);
      $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.SONIC_BOOM_SOUND_DELAY, Unit.INSTANCE, (long)TICKS_BEFORE_PLAYING_SOUND);
   ...
   protected void tick(ServerLevel $$0, Warden $$1, long $$2) {
      ...
      if (!$$1.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
         && !$$1.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
         $$1.getBrain().setMemoryWithExpiry(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN, Unit.INSTANCE, (long)(DURATION - TICKS_BEFORE_PLAYING_SOUND));
         $$1.getBrain()
            .getMemory(MemoryModuleType.ATTACK_TARGET)
            .filter($$1::canTargetEntity)
            .filter($$1x -> $$1.closerThan($$1x, 15.0, 20.0))
            .ifPresent($$2x -> {
               Vec3 $$3 = $$1.position().add($$1.getAttachments().get(EntityAttachment.WARDEN_CHEST, 0, $$1.getYRot()));
   ...
   public static void setCooldown(LivingEntity $$0, int $$1) {
      $$0.getBrain().setMemoryWithExpiry(MemoryModuleType.SONIC_BOOM_COOLDOWN, Unit.INSTANCE, (long)$$1);
   }
}
