...
public class AnimalMakeLove extends Behavior<Animal> {
   ...
   private boolean hasBreedTargetOfRightType(Animal $$0) {
      Brain<?> $$1 = $$0.getBrain();
      return $$1.hasMemoryValue(MemoryModuleType.BREED_TARGET)
         && ((AgeableMob)$$1.getMemory(MemoryModuleType.BREED_TARGET).get()).getType() == this.partnerType;
   }

   private Optional<? extends Animal> findValidBreedPartner(Animal $$0) {
      return ((NearestVisibleLivingEntities)$$0.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get()).findClosest($$1 -> {
         if ($$1.getType() == this.partnerType && $$1 instanceof Animal $$2 && $$0.canMate($$2) && !$$2.isPanicking()) {
            return true;
         }
