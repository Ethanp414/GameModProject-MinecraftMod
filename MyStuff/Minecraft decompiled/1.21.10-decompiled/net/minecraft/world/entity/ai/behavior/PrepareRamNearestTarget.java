...
public class PrepareRamNearestTarget<E extends PathfinderMob> extends Behavior<E> {
   ...
   protected void tick(ServerLevel $$0, E $$1, long $$2) {
      if (!this.ramCandidate.isEmpty()) {
         ...
         boolean $$3 = !((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get())
            .getTarget()
            .blockPosition()
         ...
         if ($$3) {
            ...
            this.chooseRamPosition($$1, ((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).target);
         } else {
            ...
            if ($$4.equals(((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getStartPosition())) {
               ...
               if ($$2 - this.reachedRamPositionTimestamp.get() >= (long)this.ramPrepareTime) {
                  $$1.getBrain()
                     .setMemory(
                        MemoryModuleType.RAM_TARGET,
                        this.getEdgeOfBlock($$4, ((PrepareRamNearestTarget.RamCandidate)this.ramCandidate.get()).getTargetPosition())
                     );
                  $$0.playSound(null, $$1, (SoundEvent)this.getPrepareRamSound.apply($$1), SoundSource.NEUTRAL, 1.0F, $$1.getVoicePitch());
   ...
   private Vec3 getEdgeOfBlock(BlockPos $$0, BlockPos $$1) {
      double $$2 = 0.5;
      double $$3 = 0.5 * (double)Mth.sign((double)($$1.getX() - $$0.getX()));
      double $$4 = 0.5 * (double)Mth.sign((double)($$1.getZ() - $$0.getZ()));
   ...
   private Optional<BlockPos> calculateRammingStartPosition(PathfinderMob $$0, LivingEntity $$1) {
      ...
      if (!this.isWalkableBlock($$0, $$2)) {
         ...
         List<BlockPos> $$3 = Lists.<BlockPos>newArrayList();
         ...
         return $$3.stream().sorted(Comparator.comparingDouble($$0.blockPosition()::distSqr)).filter($$1x -> {
            Path $$2xx = $$7.createPath($$1x, 0);
            return $$2xx != null && $$2xx.canReach();
         }).findFirst();
      }
   }
