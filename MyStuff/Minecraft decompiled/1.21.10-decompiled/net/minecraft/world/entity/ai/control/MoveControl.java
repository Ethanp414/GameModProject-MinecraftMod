...
public class MoveControl implements Control {
   ...
   public void tick() {
      if (this.operation == MoveControl.Operation.STRAFE) {
         ...
         if ($$11 > (double)this.mob.maxUpStep() && $$9 * $$9 + $$10 * $$10 < (double)Math.max(1.0F, this.mob.getBbWidth())
            || !$$16.isEmpty()
               && this.mob.getY() < $$16.max(Direction.Axis.Y) + (double)$$14.getY()
   ...
   private boolean isWalkable(float $$0, float $$1) {
      ...
      if ($$2 != null) {
         ...
         if ($$3 != null
            && $$3.getPathType(this.mob, BlockPos.containing(this.mob.getX() + (double)$$0, (double)this.mob.getBlockY(), this.mob.getZ() + (double)$$1))
               != PathType.WALKABLE) {
            return false;
         }
      }
