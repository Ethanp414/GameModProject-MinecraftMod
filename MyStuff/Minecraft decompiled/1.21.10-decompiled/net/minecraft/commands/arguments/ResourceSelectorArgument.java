...
public class ResourceSelectorArgument<T> implements ArgumentType<Collection<Holder.Reference<T>>> {
   ...
   public static final Dynamic2CommandExceptionType ERROR_NO_MATCHES = new Dynamic2CommandExceptionType(
      ($$0, $$1) -> Component.translatableEscape("argument.resource_selector.not_found", $$0, $$1)
   );
   ...
   public Collection<Holder.Reference<T>> parse(StringReader $$0) throws CommandSyntaxException {
      ...
      List<Holder.Reference<T>> $$2 = this.registryLookup.listElements().filter($$1x -> matches($$1, $$1x.key().location())).toList();
      if ($$2.isEmpty()) {
         throw ERROR_NO_MATCHES.createWithContext($$0, $$1, this.registryKey.location());
      } else {
         return $$2;
      }
   }

   public static <T> Collection<Holder.Reference<T>> parse(StringReader $$0, HolderLookup<T> $$1) {
      String $$2 = ensureNamespaced(readPattern($$0));
      return $$1.listElements().filter($$1x -> matches($$2, $$1x.key().location())).toList();
   }
   ...
   private static boolean isAllowedPatternCharacter(char $$0) {
      return ResourceLocation.isAllowedInResourceLocation($$0) || $$0 == '*' || $$0 == '?';
   }
   ...
   public static <T> Collection<Holder.Reference<T>> getSelectedResources(CommandContext<CommandSourceStack> $$0, String $$1) {
      return $$0.getArgument($$1, Collection.class);
   }
