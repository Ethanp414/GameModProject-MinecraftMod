...
public class StringRepresentableArgument<T extends Enum<T> & StringRepresentable> implements ArgumentType<T> {
   private static final DynamicCommandExceptionType ERROR_INVALID_VALUE = new DynamicCommandExceptionType(
      $$0 -> Component.translatableEscape("argument.enum.invalid", $$0)
   );
   ...
   public T parse(StringReader $$0) throws CommandSyntaxException {
      ...
      return (T)this.codec.parse(JsonOps.INSTANCE, new JsonPrimitive($$1)).result().orElseThrow(() -> ERROR_INVALID_VALUE.createWithContext($$0, $$1));
   }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> $$0, SuggestionsBuilder $$1) {
      return SharedSuggestionProvider.suggest(
         (Iterable<String>)Arrays.stream((Enum[])this.values.get())
            .map($$0x -> ((StringRepresentable)$$0x).getSerializedName())
            .map(this::convertId)
   ...
   public Collection<String> getExamples() {
      return (Collection<String>)Arrays.stream((Enum[])this.values.get())
         .map($$0 -> ((StringRepresentable)$$0).getSerializedName())
         .map(this::convertId)
         .limit(2L)
         .collect(Collectors.toList());
   }
