...
public class ExecutionContext<T> implements AutoCloseable {
   ...
   public static <T extends ExecutionCommandSource<T>> void queueInitialFunctionCall(
      ExecutionContext<T> $$0, InstantiatedFunction<T> $$1, T $$2, CommandResultCallback $$3
   ) {
   ...
   public static <T extends ExecutionCommandSource<T>> void queueInitialCommandExecution(
      ExecutionContext<T> $$0, String $$1, ContextChain<T> $$2, T $$3, CommandResultCallback $$4
   ) {
   ...
   public void discardAtDepthOrHigher(int $$0) {
      while(!this.commandQueue.isEmpty() && ((CommandQueueEntry)this.commandQueue.peek()).frame().depth() >= $$0) {
   ...
   public void runCommandQueue() {
      ...
      while(true) {
         ...
         CommandQueueEntry<T> $$0 = (CommandQueueEntry)this.commandQueue.pollFirst();
         if ($$0 == null) {
            return;
         }
   ...
   private void pushNewCommands() {
      for(int $$0 = this.newTopCommands.size() - 1; $$0 >= 0; --$$0) {
         this.commandQueue.addFirst((CommandQueueEntry)this.newTopCommands.get($$0));
      }
