...
public class BuildContexts<T extends ExecutionCommandSource<T>> {
   ...
   public static final DynamicCommandExceptionType ERROR_FORK_LIMIT_REACHED = new DynamicCommandExceptionType(
      $$0 -> Component.translatableEscape("command.forkLimit", $$0)
   );
   ...
   protected void execute(T $$0, List<T> $$1, ExecutionContext<T> $$2, Frame $$3, ChainModifiers $$4) {
      ...
      if ($$5.getStage() != Stage.EXECUTE) {
         $$2.profiler().push((Supplier<String>)(() -> "prepare " + this.commandInput));
         ...
         try {
            for(int $$8 = $$2.forkLimit(); $$5.getStage() != Stage.EXECUTE; $$5 = $$5.nextStage()) {
               ...
               if ($$10 instanceof CustomModifierExecutor $$11) {
                  ...
                  for(T $$14 : $$7) {
                     try {
                        ...
                        if ($$13.size() + $$15.size() >= $$8) {
                           $$0.handleError(ERROR_FORK_LIMIT_REACHED.create($$8), (boolean)$$11, $$2.tracer());
                           return;
                        }

                        $$13.addAll($$15);
                     } catch (CommandSyntaxException var20) {
                        $$14.handleError(var20, (boolean)$$11, $$2.tracer());
                        if ($$11 == false) {
                           return;
                        }
                     }
      ...
      if ($$7.isEmpty()) {
         ...
         Command<T> $$18 = $$17.getCommand();
         if ($$18 instanceof CustomCommandExecutor $$19) {
            ...
            if ($$6.isReturn()) {
               T $$22 = (T)$$7.get(0);
               $$22 = $$22.withCallback(CommandResultCallback.chain($$22.callback(), $$3.returnValueConsumer()));
               $$7 = List.of($$22);
            }
