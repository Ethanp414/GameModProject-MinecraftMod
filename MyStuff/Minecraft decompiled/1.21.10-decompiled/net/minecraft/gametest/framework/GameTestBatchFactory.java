...
public class GameTestBatchFactory {
   ...
   public static List<GameTestBatch> divideIntoBatches(
      Collection<Holder.Reference<GameTestInstance>> $$0, GameTestBatchFactory.TestDecorator $$1, ServerLevel $$2
   ) {
      Map<Holder<TestEnvironmentDefinition>, List<GameTestInfo>> $$3 = (Map)$$0.stream()
         .flatMap($$2x -> $$1.decorate($$2x, $$2))
         .collect(Collectors.groupingBy($$0x -> $$0x.getTest().batch()));
      return $$3.entrySet().stream().flatMap($$0x -> {
         Holder<TestEnvironmentDefinition> $$1xx = (Holder)$$0x.getKey();
         List<GameTestInfo> $$2xx = (List)$$0x.getValue();
         return Streams.mapWithIndex(Lists.partition($$2xx, 50).stream(), ($$1xx, $$2xx) -> toGameTestBatch($$1xx, $$1x, (int)$$2xx));
      }).toList();
   }
   ...
   public static GameTestRunner.GameTestBatcher fromGameTestInfo(int $$0) {
      return $$1 -> {
         Map<Holder<TestEnvironmentDefinition>, List<GameTestInfo>> $$2 = (Map)$$1.stream()
            .filter(Objects::nonNull)
            .collect(Collectors.groupingBy($$0xx -> $$0xx.getTest().batch()));
         return $$2.entrySet().stream().flatMap($$1x -> {
            Holder<TestEnvironmentDefinition> $$2xx = (Holder)$$1x.getKey();
            List<GameTestInfo> $$3 = (List)$$1x.getValue();
