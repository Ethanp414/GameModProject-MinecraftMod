...
public interface ProtocolSwapHandler {
   static void handleInboundTerminalPacket(ChannelHandlerContext $$0, Packet<?> $$1) {
      if ($$1.isTerminal()) {
         ...
         $$0.pipeline().addBefore($$0.name(), "inbound_config", new UnconfiguredPipelineHandler.Inbound());
   ...
   static void handleOutboundTerminalPacket(ChannelHandlerContext $$0, Packet<?> $$1) {
      if ($$1.isTerminal()) {
         $$0.pipeline().addAfter($$0.name(), "outbound_config", new UnconfiguredPipelineHandler.Outbound());
         $$0.pipeline().remove($$0.name());
      }
   }
}
