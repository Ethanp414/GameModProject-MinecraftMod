...
public class ComponentSerialization {
   ...
   public static Codec<Component> flatRestrictedCodec(final int $$0) {
      return new Codec<Component>() {
         ...
         private <T> boolean isTooLarge(DynamicOps<T> $$0x, Component $$1) {
            ...
            return $$2.isSuccess() && GsonHelper.encodesLongerThan($$2.getOrThrow(), $$0);
         }

         private static <T> DynamicOps<JsonElement> asJsonOps(DynamicOps<T> $$0x) {
            return (DynamicOps<JsonElement>)($$0 instanceof RegistryOps $$1 ? $$1.withParent(JsonOps.INSTANCE) : JsonOps.INSTANCE);
         }
      };
   }

   private static MutableComponent createFromList(List<Component> $$0) {
      MutableComponent $$1 = ((Component)$$0.get(0)).copy();

      for(int $$2 = 1; $$2 < $$0.size(); ++$$2) {
         $$1.append((Component)$$0.get($$2));
      }
   ...
   public static <T> MapCodec<T> createLegacyComponentMatcher(
      ExtraCodecs.LateBoundIdMapper<String, MapCodec<? extends T>> $$0, Function<T, MapCodec<? extends T>> $$1, String $$2
   ) {
   ...
   private static Codec<Component> createCodec(Codec<Component> $$0) {
      ExtraCodecs.LateBoundIdMapper<String, MapCodec<? extends ComponentContents>> $$1 = new ExtraCodecs.LateBoundIdMapper<>();
      bootstrap($$1);
      ...
      return Codec.either(Codec.either(Codec.STRING, ExtraCodecs.nonEmptyList($$0.listOf())), $$3)
         .xmap($$0x -> $$0x.map($$0xx -> $$0xx.map(Component::literal, ComponentSerialization::createFromList), $$0xx -> $$0xx), $$0x -> {
            String $$1xx = $$0x.tryCollapseToString();
            return $$1xx != null ? Either.left(Either.left($$1xx)) : Either.right($$0x);
         });
   }
   ...
   static class FuzzyCodec<T> extends MapCodec<T> {
      ...
      public <S> RecordBuilder<S> encode(T $$0, DynamicOps<S> $$1, RecordBuilder<S> $$2) {
         MapEncoder<T> $$3 = (MapEncoder)this.encoderGetter.apply($$0);
