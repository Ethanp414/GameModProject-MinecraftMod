...
public record PlayerSprite(ResolvableProfile player, boolean hat) implements ObjectInfo {
   public static final MapCodec<PlayerSprite> MAP_CODEC = RecordCodecBuilder.mapCodec(
      $$0 -> $$0.group(
               ...
               Codec.BOOL.optionalFieldOf("hat", Boolean.valueOf(true)).forGetter(PlayerSprite::hat)
            )
            .apply($$0, PlayerSprite::new)
   );
   ...
   public String description() {
      return (String)this.player.name().map($$0 -> "[" + $$0 + " head]").orElse("[unknown player head]");
   }
