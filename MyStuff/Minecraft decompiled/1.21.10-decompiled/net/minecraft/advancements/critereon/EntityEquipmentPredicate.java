...
public record EntityEquipmentPredicate(
                  ...
                  DataComponentMatchers.Builder.components()
                     .exact(
                        DataComponentExactPredicate.someOf(
                           Raid.getOminousBannerInstance($$1).getComponents(), DataComponents.BANNER_PATTERNS, DataComponents.ITEM_NAME
                        )
                     )
                     .build()
               )
   ...
   public boolean matches(@Nullable Entity $$0) {
      if ($$0 instanceof LivingEntity $$1) {
         if (this.head.isPresent() && !((ItemPredicate)this.head.get()).test($$1.getItemBySlot(EquipmentSlot.HEAD))) {
            return false;
         } else if (this.chest.isPresent() && !((ItemPredicate)this.chest.get()).test($$1.getItemBySlot(EquipmentSlot.CHEST))) {
            return false;
         } else if (this.legs.isPresent() && !((ItemPredicate)this.legs.get()).test($$1.getItemBySlot(EquipmentSlot.LEGS))) {
            return false;
         } else if (this.feet.isPresent() && !((ItemPredicate)this.feet.get()).test($$1.getItemBySlot(EquipmentSlot.FEET))) {
            return false;
         } else if (this.body.isPresent() && !((ItemPredicate)this.body.get()).test($$1.getItemBySlot(EquipmentSlot.BODY))) {
            return false;
         } else if (this.mainhand.isPresent() && !((ItemPredicate)this.mainhand.get()).test($$1.getItemBySlot(EquipmentSlot.MAINHAND))) {
            return false;
         } else {
            return !this.offhand.isPresent() || ((ItemPredicate)this.offhand.get()).test($$1.getItemBySlot(EquipmentSlot.OFFHAND));
         }
      } else {
         return false;
      }
