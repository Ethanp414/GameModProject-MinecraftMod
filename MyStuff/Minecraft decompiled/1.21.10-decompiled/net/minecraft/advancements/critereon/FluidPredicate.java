...
public record FluidPredicate(Optional<HolderSet<Fluid>> fluids, Optional<StatePropertiesPredicate> properties) {
   ...
   public boolean matches(ServerLevel $$0, BlockPos $$1) {
      if (!$$0.isLoaded($$1)) {
         ...
         if (this.fluids.isPresent() && !$$2.is((HolderSet<Fluid>)this.fluids.get())) {
            return false;
         } else {
            return !this.properties.isPresent() || ((StatePropertiesPredicate)this.properties.get()).matches($$2);
         }
      }
   }
   ...
   public static class Builder {
      ...
      public FluidPredicate.Builder of(Fluid $$0) {
         this.fluids = Optional.of(HolderSet.direct($$0.builtInRegistryHolder()));
         return this;
      }
