...
public record LocationPredicate(
   ...
   public boolean matches(ServerLevel $$0, double $$1, double $$2, double $$3) {
      if (this.position.isPresent() && !((LocationPredicate.PositionPredicate)this.position.get()).matches($$1, $$2, $$3)) {
         return false;
         ...
         if (!this.biomes.isPresent() || $$5 && ((HolderSet)this.biomes.get()).contains($$0.getBiome($$4))) {
            if (!this.structures.isPresent()
               || $$5 && $$0.structureManager().getStructureWithPieceAt($$4, (HolderSet<Structure>)this.structures.get()).isValid()) {
               if (!this.smokey.isPresent() || $$5 && this.smokey.get() == CampfireBlock.isSmokeyPos($$0, $$4)) {
                  if (this.light.isPresent() && !((LightPredicate)this.light.get()).matches($$0, $$4)) {
                     return false;
                  } else if (this.block.isPresent() && !((BlockPredicate)this.block.get()).matches($$0, $$4)) {
                     return false;
                  } else if (this.fluid.isPresent() && !((FluidPredicate)this.fluid.get()).matches($$0, $$4)) {
                     return false;
                  } else {
                     return !this.canSeeSky.isPresent() || this.canSeeSky.get() == $$0.canSeeSky($$4);
                  }
               } else {
                  return false;
               }
   ...
   public static class Builder {
      ...
      public static LocationPredicate.Builder inBiome(Holder<Biome> $$0) {
         return location().setBiomes(HolderSet.direct($$0));
      }
      ...
      public static LocationPredicate.Builder inStructure(Holder<Structure> $$0) {
         return location().setStructures(HolderSet.direct($$0));
      }
