...
public record DamagePredicate(
   ...
   public boolean matches(ServerPlayer $$0, DamageSource $$1, float $$2, float $$3, boolean $$4) {
      if (!this.dealtDamage.matches((double)$$2)) {
         return false;
      } else if (!this.takenDamage.matches((double)$$3)) {
         return false;
      } else if (this.sourceEntity.isPresent() && !((EntityPredicate)this.sourceEntity.get()).matches($$0, $$1.getEntity())) {
         return false;
      } else if (this.blocked.isPresent() && this.blocked.get() != $$4) {
         return false;
      } else {
         return !this.type.isPresent() || ((DamageSourcePredicate)this.type.get()).matches($$0, $$1);
      }
   }
