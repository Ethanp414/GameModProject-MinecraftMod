...
public interface MinMaxBounds<T extends Number & Comparable<T>> {
   ...
   public static record Bounds<T extends Number & Comparable<T>>(Optional<T> min, Optional<T> max) {
      ...
      static <T extends Number & Comparable<T>> Codec<MinMaxBounds.Bounds<T>> createCodec(Codec<T> $$0) {
         ...
         return Codec.either($$1, $$0).xmap($$0x -> $$0x.map($$0xx -> $$0xx, $$0xx -> exactly((T)$$0xx)), $$0x -> {
            Optional<T> $$1xx = $$0x.asPoint();
            return $$1xx.isPresent() ? Either.right((Number)$$1xx.get()) : Either.left($$0x);
         });
      }
      ...
      static <B extends ByteBuf, T extends Number & Comparable<T>> StreamCodec<B, MinMaxBounds.Bounds<T>> createStreamCodec(final StreamCodec<B, T> $$0) {
         return new StreamCodec<B, MinMaxBounds.Bounds<T>>() {
            ...
            public MinMaxBounds.Bounds<T> decode(B $$0x) {
               byte $$1 = $$0.readByte();
               Optional<T> $$2 = ($$1 & 1) != 0 ? Optional.of((Number)$$0.decode($$0)) : Optional.empty();
               Optional<T> $$3 = ($$1 & 2) != 0 ? Optional.of((Number)$$0.decode($$0)) : Optional.empty();
   ...
   public static record Doubles(MinMaxBounds.Bounds<Double> bounds, MinMaxBounds.Bounds<Double> boundsSqr) implements MinMaxBounds<Double> {
      ...
      public static MinMaxBounds.Doubles exactly(double $$0) {
         return new MinMaxBounds.Doubles(MinMaxBounds.Bounds.exactly((T)$$0));
      }

      public static MinMaxBounds.Doubles between(double $$0, double $$1) {
         return new MinMaxBounds.Doubles(MinMaxBounds.Bounds.between((T)$$0, (T)$$1));
      }

      public static MinMaxBounds.Doubles atLeast(double $$0) {
         return new MinMaxBounds.Doubles(MinMaxBounds.Bounds.atLeast((T)$$0));
      }

      public static MinMaxBounds.Doubles atMost(double $$0) {
         return new MinMaxBounds.Doubles(MinMaxBounds.Bounds.atMost((T)$$0));
      }

      public boolean matches(double $$0) {
         if (this.bounds.min.isPresent() && this.bounds.min.get() > $$0) {
            return false;
         } else {
            return this.bounds.max.isEmpty() || !(this.bounds.max.get() < $$0);
         }
      }

      public boolean matchesSqr(double $$0) {
         if (this.boundsSqr.min.isPresent() && this.boundsSqr.min.get() > $$0) {
            return false;
         } else {
            return this.boundsSqr.max.isEmpty() || !(this.boundsSqr.max.get() < $$0);
         }
      }
   ...
   public static record FloatDegrees(MinMaxBounds.Bounds<Float> bounds) implements MinMaxBounds<Float> {
      ...
      public static MinMaxBounds.FloatDegrees fromReader(StringReader $$0) throws CommandSyntaxException {
         MinMaxBounds.Bounds<Float> $$1 = MinMaxBounds.Bounds.fromReader(
            $$0, Float::parseFloat, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidFloat
         );
   ...
   public static record Ints(MinMaxBounds.Bounds<Integer> bounds, MinMaxBounds.Bounds<Long> boundsSqr) implements MinMaxBounds<Integer> {
      ...
      public static MinMaxBounds.Ints exactly(int $$0) {
         return new MinMaxBounds.Ints(MinMaxBounds.Bounds.exactly((T)$$0));
      }

      public static MinMaxBounds.Ints between(int $$0, int $$1) {
         return new MinMaxBounds.Ints(MinMaxBounds.Bounds.between((T)$$0, (T)$$1));
      }

      public static MinMaxBounds.Ints atLeast(int $$0) {
         return new MinMaxBounds.Ints(MinMaxBounds.Bounds.atLeast((T)$$0));
      }

      public static MinMaxBounds.Ints atMost(int $$0) {
         return new MinMaxBounds.Ints(MinMaxBounds.Bounds.atMost((T)$$0));
      }
      ...
      public static MinMaxBounds.Ints fromReader(StringReader $$0) throws CommandSyntaxException {
         int $$1 = $$0.getCursor();
         MinMaxBounds.Bounds<Integer> $$2 = MinMaxBounds.Bounds.fromReader(
            $$0, Integer::parseInt, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidInt
         );
         if ($$2.areSwapped()) {
            $$0.setCursor($$1);
            throw ERROR_SWAPPED.createWithContext($$0);
