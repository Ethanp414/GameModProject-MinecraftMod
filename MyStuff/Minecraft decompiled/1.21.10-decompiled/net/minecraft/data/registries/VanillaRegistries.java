...
public class VanillaRegistries {
   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderLookup.Provider $$0) {
      validateThatAllBiomeFeaturesHaveBiomeFilter($$0.lookupOrThrow(Registries.PLACED_FEATURE), $$0.lookupOrThrow(Registries.BIOME));
   }

   public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> $$0, HolderLookup<Biome> $$1) {
      $$1.listElements().forEach($$1x -> {
         ResourceLocation $$2 = $$1x.key().location();
         List<HolderSet<PlacedFeature>> $$3 = ((Biome)$$1x.value()).getGenerationSettings().features();
         $$3.stream().flatMap(HolderSet::stream).forEach($$3x -> $$3x.unwrap().ifLeft($$2xx -> {
               Holder.Reference<PlacedFeature> $$3xxx = $$0.getOrThrow($$2xx);
               if (!validatePlacedFeature((PlacedFeature)$$3xxx.value())) {
                  Util.logAndPauseIfInIde("Placed feature " + $$2xx.location() + " in biome " + $$2 + " is missing BiomeFilter.biome()");
               }
            }).ifRight($$1xxx -> {
               if (!validatePlacedFeature($$1xxx)) {
                  Util.logAndPauseIfInIde("Placed inline feature in biome " + $$1x + " is missing BiomeFilter.biome()");
               }
            }));
      });
   }
