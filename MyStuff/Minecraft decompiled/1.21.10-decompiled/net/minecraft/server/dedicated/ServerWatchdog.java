      ...
      while(this.server.isRunning()) {
         ...
         if ($$2 > this.maxTickTimeNanos) {
            LOGGER.error(
               ...
               String.format(Locale.ROOT, "%.2f", (float)$$2 / (float)TimeUtil.NANOSECONDS_PER_SECOND),
            ...
            CrashReport $$3 = createWatchdogCrashReport("Watching Server", this.server.getRunningThread().threadId());
            ...
            $$4.setDetail(
               ...
               (CrashReportDetail<String>)(() -> (String)Streams.stream(this.server.getAllLevels())
                     .map($$0x -> $$0x.dimension().location() + ": " + $$0x.getWatchdogStats())
            ...
            if ($$3.saveToFile($$5, ReportType.CRASH)) {
               LOGGER.error("This crash report has been saved to: {}", $$5.toAbsolutePath());
            } else {
