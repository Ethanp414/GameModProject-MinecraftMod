   ...
   public static <T extends ExecutionCommandSource<T>> void register(CommandDispatcher<T> $$0) {
      $$0.register(
         ((LiteralArgumentBuilder)LiteralArgumentBuilder.literal("return").requires(Commands.hasPermission(2)))
            .then(RequiredArgumentBuilder.<T, T>argument("value", IntegerArgumentType.integer()).executes(new ReturnCommand.ReturnValueCustomExecutor<>()))
            .then(LiteralArgumentBuilder.<T>literal("fail").executes(new ReturnCommand.ReturnFailCustomExecutor<>()))
            .then(LiteralArgumentBuilder.<T>literal("run").forward($$0.getRoot(), new ReturnCommand.ReturnFromCommandCustomModifier<>(), false))
      );
   }
   ...
   static class ReturnFromCommandCustomModifier<T extends ExecutionCommandSource<T>> implements CustomModifierExecutor.ModifierAdapter<T> {
      public void apply(T $$0, List<T> $$1, ContextChain<T> $$2, ChainModifiers $$3, ExecutionControl<T> $$4) {
         if ($$1.isEmpty()) {
            ...
            $$4.queueNext(new BuildContexts.Continuation<>($$6, $$5, $$3.setReturn(), $$0, $$1));
         }
      }
   }
