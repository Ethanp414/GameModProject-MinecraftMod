...
public class FunctionCommand {
   ...
   static final FunctionCommand.Callbacks<CommandSourceStack> FULL_CONTEXT_CALLBACKS = new FunctionCommand.Callbacks<CommandSourceStack>() {
      public void signalResult(CommandSourceStack $$0, ResourceLocation $$1, int $$2) {
         $$0.sendSuccess(() -> Component.translatable("commands.function.result", Component.translationArg($$1), $$2), true);
      }
   };
   ...
   private static <T extends ExecutionCommandSource<T>> CommandResultCallback decorateOutputIfNeeded(
      T $$0, FunctionCommand.Callbacks<T> $$1, ResourceLocation $$2, CommandResultCallback $$3
   ) {
   ...
   abstract static class FunctionCustomExecutor
      ...
      public void runGuarded(CommandSourceStack $$0, ContextChain<CommandSourceStack> $$1, ChainModifiers $$2, ExecutionControl<CommandSourceStack> $$3) throws CommandSyntaxException {
         ...
         Pair<ResourceLocation, Collection<CommandFunction<CommandSourceStack>>> $$5 = FunctionArgument.getFunctionCollection($$4, "name");
         Collection<CommandFunction<CommandSourceStack>> $$6 = (Collection)$$5.getSecond();
         if ($$6.isEmpty()) {
            throw FunctionCommand.ERROR_NO_FUNCTIONS.create(Component.translationArg($$5.getFirst()));
         } else {
            CompoundTag $$7 = this.arguments($$4);
            CommandSourceStack $$8 = FunctionCommand.modifySenderForExecution($$0);
            if ($$6.size() == 1) {
               $$0.sendSuccess(
                  () -> Component.translatable("commands.function.scheduled.single", Component.translationArg(((CommandFunction)$$6.iterator().next()).id())),
                  true
               );
            } else {
               $$0.sendSuccess(
                  () -> Component.translatable(
                        ...
                        ComponentUtils.formatList($$6.stream().map(CommandFunction::id).toList(), Component::translationArg)
                     ),
                  true
               );
            }
