...
public class RecipeCommand {
                           ...
                           Commands.argument("recipe", ResourceKeyArgument.key(Registries.RECIPE))
                              .executes(
                                 $$0x -> giveRecipes(
                                       ...
                                       Collections.singleton(ResourceKeyArgument.getRecipe($$0x, "recipe"))
                                    )
                              )
                        )
                        .then(
                           Commands.literal("*")
                              .executes(
                                 $$0x -> giveRecipes(
                                       (CommandSourceStack)$$0x.getSource(),
                                       EntityArgument.getPlayers($$0x, "targets"),
                                       ((CommandSourceStack)$$0x.getSource()).getServer().getRecipeManager().getRecipes()
                                    )
                              )
                        )
                  )
   ...
   private static int giveRecipes(CommandSourceStack $$0, Collection<ServerPlayer> $$1, Collection<RecipeHolder<?>> $$2) throws CommandSyntaxException {
      ...
      if ($$3 == 0) {
         ...
         if ($$1.size() == 1) {
            $$0.sendSuccess(
               () -> Component.translatable("commands.recipe.give.success.single", $$2.size(), ((ServerPlayer)$$1.iterator().next()).getDisplayName()), true
            );
         } else {
   ...
   private static int takeRecipes(CommandSourceStack $$0, Collection<ServerPlayer> $$1, Collection<RecipeHolder<?>> $$2) throws CommandSyntaxException {
      ...
      if ($$3 == 0) {
         ...
         if ($$1.size() == 1) {
            $$0.sendSuccess(
               () -> Component.translatable("commands.recipe.take.success.single", $$2.size(), ((ServerPlayer)$$1.iterator().next()).getDisplayName()), true
            );
         } else {
