...
public class AdvancementCommands {
   ...
   private static final Dynamic2CommandExceptionType ERROR_CRITERION_NOT_FOUND = new Dynamic2CommandExceptionType(
      ($$0, $$1) -> Component.translatableEscape("commands.advancement.criterionNotFound", $$0, $$1)
   );
                                 ...
                                 ((RequiredArgumentBuilder)Commands.argument("advancement", ResourceKeyArgument.key(Registries.ADVANCEMENT))
                                       .executes(
                                          $$0x -> perform(
                                                ...
                                                getAdvancements($$0x, ResourceKeyArgument.getAdvancement($$0x, "advancement"), AdvancementCommands.Mode.ONLY)
                                             )
                                       ))
                                    .then(
            ...
            if ($$1.size() == 1) {
               throw ERROR_NO_ACTION_PERFORMED.create(
                  Component.translatable(
                     $$2.getKey() + ".one.to.one.failure",
                     Advancement.name((AdvancementHolder)$$3.iterator().next()),
                     ((ServerPlayer)$$1.iterator().next()).getDisplayName()
                  )
               );
            } else {
               throw ERROR_NO_ACTION_PERFORMED.create(
                  Component.translatable($$2.getKey() + ".one.to.many.failure", Advancement.name((AdvancementHolder)$$3.iterator().next()), $$1.size())
               );
            }
         } else if ($$1.size() == 1) {
            throw ERROR_NO_ACTION_PERFORMED.create(
               Component.translatable($$2.getKey() + ".many.to.one.failure", $$3.size(), ((ServerPlayer)$$1.iterator().next()).getDisplayName())
            );
         } else {
            throw ERROR_NO_ACTION_PERFORMED.create(Component.translatable($$2.getKey() + ".many.to.many.failure", $$3.size(), $$1.size()));
         }
      } else {
         if ($$3.size() == 1) {
            if ($$1.size() == 1) {
               ...
               $$0.sendSuccess(
                  () -> Component.translatable($$2.getKey() + ".one.to.many.success", Advancement.name((AdvancementHolder)$$3.iterator().next()), $$1.size()),
                  true
               );
            }
         } else if ($$1.size() == 1) {
            $$0.sendSuccess(
               () -> Component.translatable($$2.getKey() + ".many.to.one.success", $$3.size(), ((ServerPlayer)$$1.iterator().next()).getDisplayName()), true
            );
         } else {
      ...
      if (!$$6.criteria().containsKey($$4)) {
         ...
         if ($$5 == 0) {
            if ($$1.size() == 1) {
               throw ERROR_NO_ACTION_PERFORMED.create(
                  Component.translatable(
                     $$2.getKey() + ".criterion.to.one.failure", $$4, Advancement.name($$3), ((ServerPlayer)$$1.iterator().next()).getDisplayName()
                  )
               );
            } else {
               throw ERROR_NO_ACTION_PERFORMED.create(
                  Component.translatable($$2.getKey() + ".criterion.to.many.failure", $$4, Advancement.name($$3), $$1.size())
               );
            }
         } else {
            if ($$1.size() == 1) {
               $$0.sendSuccess(
                  () -> Component.translatable(
                        $$2.getKey() + ".criterion.to.one.success", $$4, Advancement.name($$3), ((ServerPlayer)$$1.iterator().next()).getDisplayName()
                     ),
                  true
               );
            } else {
               $$0.sendSuccess(() -> Component.translatable($$2.getKey() + ".criterion.to.many.success", $$4, Advancement.name($$3), $$1.size()), true);
            }
   ...
   private static List<AdvancementHolder> getAdvancements(CommandContext<CommandSourceStack> $$0, AdvancementHolder $$1, AdvancementCommands.Mode $$2) {
      AdvancementTree $$3 = $$0.getSource().getServer().getAdvancements().tree();
