...
public class PackRepository {
   ...
   public static String displayPackList(Collection<Pack> $$0) {
      return (String)$$0.stream()
         .map($$0x -> $$0x.getId() + ($$0x.getCompatibility().isCompatible() ? "" : " (incompatible)"))
         .collect(Collectors.joining(", "));
   }

   public void reload() {
      List<String> $$0 = (List)this.selected.stream().map(Pack::getId).collect(ImmutableList.toImmutableList());
   ...
   public boolean addPack(String $$0) {
      Pack $$1 = (Pack)this.available.get($$0);
      if ($$1 != null && !this.selected.contains($$1)) {
         List<Pack> $$2 = Lists.<Pack>newArrayList(this.selected);
         $$2.add($$1);
         this.selected = $$2;
         return true;
      } else {
   ...
   private List<Pack> rebuildSelected(Collection<String> $$0) {
      List<Pack> $$1 = (List)this.getAvailablePacks($$0).collect(Util.toMutableList());
   ...
   public Pack getPack(String $$0) {
      return (Pack)this.available.get($$0);
   }
