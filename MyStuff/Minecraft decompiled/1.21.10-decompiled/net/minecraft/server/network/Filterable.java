...
public record Filterable<T>(T raw, Optional<T> filtered) {
   ...
   public static Filterable<String> from(FilteredText $$0) {
      return new Filterable((T)$$0.raw(), $$0.isFiltered() ? Optional.of($$0.filteredOrEmpty()) : Optional.empty());
   }

   public T get(boolean $$0) {
      return (T)($$0 ? this.filtered.orElse(this.raw) : this.raw);
   }
   ...
   public <U> Optional<Filterable<U>> resolve(Function<T, Optional<U>> $$0) {
      Optional<U> $$1 = (Optional)$$0.apply(this.raw);
      if ($$1.isEmpty()) {
         return Optional.empty();
      } else if (this.filtered.isPresent()) {
         Optional<U> $$2 = (Optional)$$0.apply(this.filtered.get());
         return $$2.isEmpty() ? Optional.empty() : Optional.of(new Filterable<Object>($$1.get(), $$2));
      } else {
         return Optional.of(new Filterable<Object>($$1.get(), Optional.empty()));
      }
   }
}
