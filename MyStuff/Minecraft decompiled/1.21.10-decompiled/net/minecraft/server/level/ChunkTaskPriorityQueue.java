...
public class ChunkTaskPriorityQueue {
   ...
   protected void resortChunkTasks(int $$0, ChunkPos $$1, int $$2) {
      if ($$0 < PRIORITY_LEVEL_COUNT) {
         Long2ObjectLinkedOpenHashMap<List<Runnable>> $$3 = (Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get($$0);
         List<Runnable> $$4 = (List)$$3.remove($$1.toLong());
         ...
         if ($$4 != null && !$$4.isEmpty()) {
            ((List)((Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get($$2))
                  .computeIfAbsent($$1.toLong(), (Long2ObjectFunction)($$0x -> Lists.newArrayList())))
               .addAll($$4);
   ...
   protected void submit(Runnable $$0, long $$1, int $$2) {
      ((List)((Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get($$2)).computeIfAbsent($$1, (Long2ObjectFunction)($$0x -> Lists.newArrayList())))
         .add($$0);
      this.topPriorityQueueIndex = Math.min(this.topPriorityQueueIndex, $$2);
   }
   ...
   public ChunkTaskPriorityQueue.TasksForChunk pop() {
      if (!this.hasWork()) {
         ...
         Long2ObjectLinkedOpenHashMap<List<Runnable>> $$1 = (Long2ObjectLinkedOpenHashMap)this.queuesPerPriority.get($$0);
         long $$2 = $$1.firstLongKey();
         List<Runnable> $$3 = (List)$$1.removeFirst();
