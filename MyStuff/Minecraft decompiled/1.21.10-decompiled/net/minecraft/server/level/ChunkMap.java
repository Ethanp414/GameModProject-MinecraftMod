...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
   ...
   public ChunkHolder getUpdatingChunkIfPresent(long $$0) {
      return this.updatingChunkMap.get($$0);
   }

   @Nullable
   protected ChunkHolder getVisibleChunkIfPresent(long $$0) {
      return this.visibleChunkMap.get($$0);
   }
   ...
   CompletableFuture<ChunkResult<List<ChunkAccess>>> getChunkRangeFuture(ChunkHolder $$0, int $$1, IntFunction<ChunkStatus> $$2) {
      if ($$1 == 0) {
         ChunkStatus $$3 = (ChunkStatus)$$2.apply(0);
         ...
         for(int $$7 = -$$1; $$7 <= $$1; ++$$7) {
            for(int $$8 = -$$1; $$8 <= $$1; ++$$8) {
               ...
               ChunkStatus $$12 = (ChunkStatus)$$2.apply($$9);
               $$5.add($$11.scheduleChunkGenerationTask($$12, this));
            }
         }

         return Util.sequence($$5).thenApply($$0x -> {
            List<ChunkAccess> $$1xx = new ArrayList($$0x.size());

            for(ChunkResult<ChunkAccess> $$2xx : $$0x) {
               if ($$2xx == null) {
                  throw this.debugFuturesAndCreateReportedException(new IllegalStateException("At least one of the chunk futures were null"), "n/a");
               }

               ChunkAccess $$3 = $$2xx.orElse(null);
               if ($$3 == null) {
   ...
   public ReportedException debugFuturesAndCreateReportedException(IllegalStateException $$0, String $$1) {
      ...
      Consumer<ChunkHolder> $$3 = $$1x -> $$1x.getAllFutures().forEach($$2x -> {
            ChunkStatus $$3xx = (ChunkStatus)$$2x.getFirst();
            CompletableFuture<ChunkResult<ChunkAccess>> $$4xx = (CompletableFuture)$$2x.getSecond();
            if ($$4xx != null && $$4xx.isDone() && $$4xx.join() == null) {
               $$2.append($$1x.getPos()).append(" - status: ").append($$3xx).append(" future: ").append($$4xx).append(System.lineSeparator());
            }
         });
   ...
   ChunkHolder updateChunkScheduling(long $$0, int $$1, @Nullable ChunkHolder $$2, int $$3) {
      if (!ChunkLevel.isLoaded($$3) && !ChunkLevel.isLoaded($$1)) {
         ...
         if (ChunkLevel.isLoaded($$1) && $$2 == null) {
            $$2 = this.pendingUnloads.remove($$0);
            if ($$2 != null) {
               $$2.setTicketLevel($$1);
            } else {
   ...
   protected void saveAllChunks(boolean $$0) {
      if ($$0) {
         ...
         do {
            ...
            }).filter($$0x -> $$0x instanceof ImposterProtoChunk || $$0x instanceof LevelChunk).filter(this::save).forEach($$1 -> $$2.setTrue());
         } while($$2.isTrue());
   ...
   private void processUnloads(BooleanSupplier $$0) {
      ...
      for(LongIterator $$1 = this.toDrop.iterator(); $$1.hasNext(); $$1.remove()) {
         $$2 = $$1.nextLong();
         ChunkHolder $$3 = this.updatingChunkMap.get($$2);
         if ($$3 != null) {
      ...
      while(($$4 > 0 || $$0.getAsBoolean()) && ($$2 = (long)((Runnable)this.unloadQueue.poll())) != null) {
         --$$4;
         $$2.run();
      }
   ...
   private void saveChunksEagerly(BooleanSupplier $$0) {
      ...
      while($$2 < 20 && this.activeChunkWrites.get() < 128 && $$0.getAsBoolean() && $$3.hasNext()) {
         long $$4 = $$3.nextLong();
         ChunkHolder $$5 = this.visibleChunkMap.get($$4);
   ...
   private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos $$0) {
      CompletableFuture<Optional<SerializableChunkData>> $$1 = this.readChunk($$0).thenApplyAsync($$1x -> $$1x.map($$1xx -> {
            SerializableChunkData $$2xx = SerializableChunkData.parse(this.level, this.level.palettedContainerFactory(), $$1xx);
            if ($$2xx == null) {
      ...
      return $$1.thenCombine($$2, ($$0x, $$1x) -> $$0x).thenApplyAsync($$1x -> {
         Profiler.get().incrementCounter("chunkLoad");
         if ($$1x.isPresent()) {
            ChunkAccess $$2xx = ((SerializableChunkData)$$1x.get()).read(this.level, this.poiManager, this.storageInfo(), $$0);
            this.markPosition($$0, $$2xx.getPersistedStatus().getChunkType());
            return $$2xx;
         } else {
   ...
   public GenerationChunkHolder acquireGeneration(long $$0) {
      ChunkHolder $$1 = this.updatingChunkMap.get($$0);
   ...
   public CompletableFuture<ChunkAccess> applyStep(GenerationChunkHolder $$0, ChunkStep $$1, StaticCache2D<GenerationChunkHolder> $$2) {
      ...
      if ($$1.targetStatus() == ChunkStatus.EMPTY) {
         ...
         try {
            ...
            $$8.setDetail("Status being generated", (CrashReportDetail<String>)(() -> $$1.targetStatus().getName()));
   ...
   private void runGenerationTask(ChunkGenerationTask $$0) {
      ...
      this.worldgenTaskDispatcher.submit(() -> {
         CompletableFuture<?> $$1xx = $$0.runUntilWait();
         if ($$1xx != null) {
            $$1xx.thenRun(() -> this.runGenerationTask($$0));
         }
   ...
   private boolean save(ChunkAccess $$0) {
      ...
      if (!$$0.tryMarkSaved()) {
         ...
         try {
            ...
            this.write($$1, $$4::join).handle(($$1x, $$2) -> {
               if ($$2 != null) {
                  this.level.getServer().reportChunkSaveFailure($$2, this.storageInfo(), $$1);
               }
   ...
   private boolean isExistingChunkFull(ChunkPos $$0) {
      ...
      if ($$1 != 0) {
         ...
         CompoundTag $$2;
         try {
            $$2 = (CompoundTag)((Optional)this.readChunk($$0).join()).orElse(null);
            if ($$2 == null) {
               this.markPositionReplaceable($$0);
               return false;
            }
   ...
   void dumpChunks(Writer $$0) throws IOException {
      ...
      for(Entry<ChunkHolder> $$2 : this.visibleChunkMap.long2ObjectEntrySet()) {
         long $$3 = $$2.getLongKey();
         ...
         $$1.writeRow(
            ...
            $$7.map($$0x -> $$0x.getBlockEntities().size()).orElse(0),
            this.ticketStorage.getTicketDebugString($$3, true),
            this.distanceManager.getChunkLevel($$3, true),
            $$7.map($$0x -> $$0x.getBlockTicks().count()).orElse(0),
            $$7.map($$0x -> $$0x.getFluidTicks().count()).orElse(0)
         );
      }
   }
   ...
   private static String printFuture(CompletableFuture<ChunkResult<LevelChunk>> $$0) {
      try {
         ChunkResult<LevelChunk> $$1 = (ChunkResult)$$0.getNow(null);
         if ($$1 != null) {
   ...
   void collectSpawningChunks(List<LevelChunk> $$0) {
      ...
      while($$1.hasNext()) {
         ChunkHolder $$2 = this.visibleChunkMap.get($$1.nextLong());
         if ($$2 != null) {
   ...
   void forEachBlockTickingChunk(Consumer<LevelChunk> $$0) {
      this.distanceManager.forEachEntityTickingChunk($$1 -> {
         ChunkHolder $$2 = this.visibleChunkMap.get($$1);
         if ($$2 != null) {
            LevelChunk $$3 = $$2.getTickingChunk();
            if ($$3 != null) {
               $$0.accept($$3);
   ...
   private static double euclideanDistanceSquared(ChunkPos $$0, Vec3 $$1) {
      double $$2 = (double)SectionPos.sectionToBlockCoord($$0.x, 8);
      double $$3 = (double)SectionPos.sectionToBlockCoord($$0.z, 8);
      double $$4 = $$2 - $$1.x;
      double $$5 = $$3 - $$1.z;
      return $$4 * $$4 + $$5 * $$5;
   }
   ...
   public void move(ServerPlayer $$0) {
      for(ChunkMap.TrackedEntity $$1 : this.entityMap.values()) {
         if ($$1.entity == $$0) {
   ...
   private void updateChunkTracking(ServerPlayer $$0) {
      ...
      ChunkTrackingView var5 = $$0.getChunkTrackingView();
      if (var5 instanceof ChunkTrackingView.Positioned $$3 && $$3.center().equals($$1) && $$3.viewDistance() == $$2) {
         return;
      }
         ...
         if ($$2 != 0) {
            ...
            if (this.entityMap.containsKey($$0.getId())) {
               ...
               if ($$0 instanceof ServerPlayer $$5) {
                  ...
                  for(ChunkMap.TrackedEntity $$6 : this.entityMap.values()) {
                     if ($$6.entity != $$5) {
                        $$6.updatePlayer($$5);
                     }
                  }
   ...
   protected void removeEntity(Entity $$0) {
      if ($$0 instanceof ServerPlayer $$1) {
         ...
         for(ChunkMap.TrackedEntity $$2 : this.entityMap.values()) {
            $$2.removePlayer($$1);
         }
      }

      ChunkMap.TrackedEntity $$3 = this.entityMap.remove($$0.getId());
      if ($$3 != null) {
         $$3.broadcastRemoved();
      }
   }
   ...
   protected void tick() {
      ...
      List<ServerPlayer> $$1 = Lists.<ServerPlayer>newArrayList();
      List<ServerPlayer> $$2 = this.level.players();

      for(ChunkMap.TrackedEntity $$3 : this.entityMap.values()) {
         ...
         if ($$6 || this.distanceManager.inEntityTickingRange($$5.chunk().toLong())) {
            $$3.serverEntity.sendChanges();
         }
      }

      if (!$$1.isEmpty()) {
         for(ChunkMap.TrackedEntity $$8 : this.entityMap.values()) {
            $$8.updatePlayers($$1);
         }
      }
   }
   ...
   public void sendToTrackingPlayers(Entity $$0, Packet<? super ClientGamePacketListener> $$1) {
      ChunkMap.TrackedEntity $$2 = this.entityMap.get($$0.getId());
      if ($$2 != null) {
   ...
   public void sendToTrackingPlayersFiltered(Entity $$0, Packet<? super ClientGamePacketListener> $$1, Predicate<ServerPlayer> $$2) {
      ChunkMap.TrackedEntity $$3 = this.entityMap.get($$0.getId());
      if ($$3 != null) {
   ...
   public boolean isTrackedByAnyPlayer(Entity $$0) {
      ChunkMap.TrackedEntity $$1 = this.entityMap.get($$0.getId());
      if ($$1 != null) {
   ...
   public void resendBiomesForChunks(List<ChunkAccess> $$0) {
      ...
      for(ChunkAccess $$2 : $$0) {
         ...
         for(ServerPlayer $$7 : this.getPlayers($$3, false)) {
            ((List)$$1.computeIfAbsent($$7, $$0x -> new ArrayList())).add($$5);
         }
      }
   ...
   public void waitForLightBeforeSending(ChunkPos $$0, int $$1) {
      ...
      ChunkPos.rangeClosed($$0, $$2).forEach($$0x -> {
         ChunkHolder $$1xx = this.getVisibleChunkIfPresent($$0x.toLong());
         if ($$1xx != null) {
            $$1xx.addSendDependency(this.lightEngine.waitForPendingTasks($$0x.x, $$0x.z));
         }
      });
   }
   ...
   class DistanceManager extends net.minecraft.server.level.DistanceManager {
      protected DistanceManager(final TicketStorage param2, final Executor param3, final Executor param4) {
         super($$0, $$1, $$2);
      }
   ...
   class TrackedEntity implements ServerEntity.Synchronizer {
      ...
      public boolean equals(Object $$0) {
         if ($$0 instanceof ChunkMap.TrackedEntity) {
            return ((ChunkMap.TrackedEntity)$$0).entity.getId() == this.entity.getId();
         } else {
            return false;
         }
      }
      ...
      public void updatePlayer(ServerPlayer $$0) {
         if ($$0 != this.entity) {
            ...
            double $$3 = (double)Math.min(this.getEffectiveRange(), $$2 * 16);
