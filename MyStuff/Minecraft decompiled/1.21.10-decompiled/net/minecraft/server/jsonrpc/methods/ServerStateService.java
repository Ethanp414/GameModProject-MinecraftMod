...
public class ServerStateService {
   public static ServerStateService.ServerState status(MinecraftApi $$0) {
      return !$$0.serverStateService().isReady()
         ...
         : new ServerStateService.ServerState(true, PlayerService.get($$0), ServerStatus.Version.current());
   }
   ...
   public static boolean stop(MinecraftApi $$0, ClientInfo $$1) {
      $$0.submit((Runnable)(() -> $$0.serverStateService().halt(false, $$1)));
      return true;
   }

   public static boolean systemMessage(MinecraftApi $$0, ServerStateService.SystemMessage $$1, ClientInfo $$2) {
      Component $$3 = (Component)$$1.message().asComponent().orElse(null);
      if ($$3 == null) {
         return false;
      } else {
         if ($$1.receivingPlayers().isPresent()) {
            if (((List)$$1.receivingPlayers().get()).isEmpty()) {
               return false;
            }

            for(PlayerDto $$4 : (List)$$1.receivingPlayers().get()) {
               ServerPlayer $$5;
               if ($$4.id().isPresent()) {
                  $$5 = $$0.playerListService().getPlayer((UUID)$$4.id().get());
               } else {
                  if (!$$4.name().isPresent()) {
                     continue;
                  }

                  $$5 = $$0.playerListService().getPlayerByName((String)$$4.name().get());
               }
