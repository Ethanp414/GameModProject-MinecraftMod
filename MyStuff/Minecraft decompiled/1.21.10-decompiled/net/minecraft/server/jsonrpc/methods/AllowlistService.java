...
public class AllowlistService {
   ...
   public static List<PlayerDto> set(MinecraftApi $$0, List<PlayerDto> $$1, ClientInfo $$2) {
      List<CompletableFuture<Optional<NameAndId>>> $$3 = $$1.stream().map($$1x -> $$0.playerListService().getUser($$1x.id(), $$1x.name())).toList();
      Set<NameAndId> $$4 = (Set)((List)Util.sequence($$3).join()).stream().filter(Optional::isPresent).map(Optional::get).collect(Collectors.toSet());
      Set<NameAndId> $$5 = (Set)$$0.allowListService().getEntries().stream().map(StoredUserEntry::getUser).collect(Collectors.toSet());
