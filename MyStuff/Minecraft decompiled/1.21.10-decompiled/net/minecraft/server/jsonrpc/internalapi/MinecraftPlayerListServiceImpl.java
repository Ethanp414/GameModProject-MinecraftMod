...
public class MinecraftPlayerListServiceImpl implements MinecraftPlayerListService {
   ...
   public Optional<ServerPlayer> getPlayer(Optional<UUID> $$0, Optional<String> $$1) {
      if ($$0.isPresent()) {
         return Optional.ofNullable(this.server.getPlayerList().getPlayer((UUID)$$0.get()));
      } else {
         return $$1.isPresent() ? Optional.ofNullable(this.server.getPlayerList().getPlayerByName((String)$$1.get())) : Optional.empty();
      }
   }
