...
public interface MinecraftPlayerListService {
   ...
   default CompletableFuture<Optional<NameAndId>> getUser(Optional<UUID> $$0, Optional<String> $$1) {
      if ($$0.isPresent()) {
         Optional<NameAndId> $$2 = this.getCachedUserById((UUID)$$0.get());
         return $$2.isPresent()
            ? CompletableFuture.completedFuture($$2)
            : CompletableFuture.supplyAsync(() -> this.fetchUserById((UUID)$$0.get()), Util.nonCriticalIoPool());
      } else {
         return $$1.isPresent()
            ? CompletableFuture.supplyAsync(() -> this.fetchUserByName((String)$$1.get()), Util.nonCriticalIoPool())
