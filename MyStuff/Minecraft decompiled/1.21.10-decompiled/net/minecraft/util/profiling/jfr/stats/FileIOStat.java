...
public record FileIOStat(Duration duration, @Nullable String path, long bytes) {
   public static FileIOStat.Summary summary(Duration $$0, List<FileIOStat> $$1) {
      ...
      return new FileIOStat.Summary(
         ...
         (Duration)$$1.stream().map(FileIOStat::duration).reduce(Duration.ZERO, Duration::plus),
         ((Map)$$1.stream().filter($$0x -> $$0x.path != null).collect(Collectors.groupingBy($$0x -> $$0x.path, Collectors.summingLong($$0x -> $$0x.bytes))))
            .entrySet()
            .stream()
