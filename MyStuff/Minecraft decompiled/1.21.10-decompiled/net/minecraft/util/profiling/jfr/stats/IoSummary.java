...
public final class IoSummary<T> {
   ...
   public IoSummary(Duration $$0, List<Pair<T, IoSummary.CountAndSize>> $$1) {
      this.recordingDuration = $$0;
      this.totalCountAndSize = (IoSummary.CountAndSize)$$1.stream()
         .map(Pair::getSecond)
         .reduce(new IoSummary.CountAndSize(0L, 0L), IoSummary.CountAndSize::add);
      this.largestSizeContributors = $$1.stream().sorted(Comparator.comparing(Pair::getSecond, IoSummary.CountAndSize.SIZE_THEN_COUNT)).limit(10L).toList();
   }
   ...
   public static record CountAndSize(long totalCount, long totalSize) {
      ...
      IoSummary.CountAndSize add(IoSummary.CountAndSize $$0) {
         return new IoSummary.CountAndSize(this.totalCount + $$0.totalCount, this.totalSize + $$0.totalSize);
      }
