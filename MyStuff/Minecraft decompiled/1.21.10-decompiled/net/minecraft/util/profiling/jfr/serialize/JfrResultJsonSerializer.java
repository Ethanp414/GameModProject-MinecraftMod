...
public class JfrResultJsonSerializer {
   ...
   private JsonElement structureGen(List<StructureGenStat> $$0) {
      ...
      $$1.add("structure", $$3);
      ((Map)$$0.stream().collect(Collectors.groupingBy(StructureGenStat::structureName))).forEach(($$3x, $$4) -> {
         JsonObject $$5 = new JsonObject();
         $$3.add($$5);
         $$5.addProperty("name", $$3x);
         TimedStatSummary<StructureGenStat> $$6 = TimedStatSummary.summary($$4);
         $$5.addProperty("count", $$6.count());
         $$5.addProperty("durationNanosTotal", $$6.totalDuration().toNanos());
         $$5.addProperty("durationNanosAvg", $$6.totalDuration().toNanos() / (long)$$6.count());
         JsonObject $$7 = Util.make(new JsonObject(), $$1xx -> $$5.add("durationNanosPercentiles", $$1xx));
         $$6.percentilesNanos().forEach(($$1xx, $$2xx) -> $$7.addProperty("p" + $$1xx, $$2xx));
         Function<StructureGenStat, JsonElement> $$8 = $$0xx -> {
            JsonObject $$1xxx = new JsonObject();
            $$1xxx.addProperty("durationNanos", $$0xx.duration().toNanos());
            $$1xxx.addProperty("chunkPosX", $$0xx.chunkPos().x);
            $$1xxx.addProperty("chunkPosZ", $$0xx.chunkPos().z);
            $$1xxx.addProperty("structureName", $$0xx.structureName());
            $$1xxx.addProperty("level", $$0xx.level());
            $$1xxx.addProperty("success", $$0xx.success());
            return $$1xxx;
         };
         $$1.add("fastest", (JsonElement)$$8.apply($$2.fastest()));
         $$1.add("slowest", (JsonElement)$$8.apply($$2.slowest()));
         $$1.add("secondSlowest", (JsonElement)($$2.secondSlowest() != null ? (JsonElement)$$8.apply($$2.secondSlowest()) : JsonNull.INSTANCE));
      });
      return $$1;
   }
   ...
   private JsonElement chunkGen(List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> $$0) {
      JsonObject $$1 = new JsonObject();
      $$1.addProperty("durationNanosTotal", $$0.stream().mapToDouble($$0x -> (double)((TimedStatSummary)$$0x.getSecond()).totalDuration().toNanos()).sum());
      JsonArray $$2 = Util.make(new JsonArray(), $$1x -> $$1.add("status", $$1x));
      ...
      for(Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>> $$3 : $$0) {
         ...
         $$5.addProperty("state", $$3.getFirst().toString());
         ...
         JsonObject $$6 = Util.make(new JsonObject(), $$1x -> $$5.add("durationNanosPercentiles", $$1x));
         ...
         Function<ChunkGenStat, JsonElement> $$7 = $$0x -> {
            ...
            $$1xx.addProperty("durationNanos", $$0x.duration().toNanos());
            $$1xx.addProperty("level", $$0x.level());
            $$1xx.addProperty("chunkPosX", $$0x.chunkPos().x);
            $$1xx.addProperty("chunkPosZ", $$0x.chunkPos().z);
            $$1xx.addProperty("worldPosX", $$0x.worldPos().x());
            $$1xx.addProperty("worldPosZ", $$0x.worldPos().z());
            return $$1xx;
         };
   ...
   private JsonElement threadAllocations(ThreadAllocationStat.Summary $$0) {
      ...
      $$0.allocationsPerSecondByThread().forEach(($$1x, $$2) -> $$1.add(Util.make(new JsonObject(), $$2x -> {
   ...
   private JsonElement serverTicks(List<TickTimeStat> $$0) {
      if ($$0.isEmpty()) {
         ...
         JsonObject $$1 = new JsonObject();
         double[] $$2 = $$0.stream().mapToDouble($$0x -> (double)$$0x.currentAverage().toNanos() / 1000000.0).toArray();
   ...
   private JsonElement fileIoSummary(FileIOStat.Summary $$0) {
      ...
      $$0.topTenContributorsByTotalBytes().forEach($$1x -> {
         ...
         $$2.add($$2xx);
         $$2xx.addProperty("path", (String)$$1x.getFirst());
         $$2xx.addProperty("totalBytes", (Number)$$1x.getSecond());
      });
      return $$1;
   }
   ...
   private <T> JsonElement ioSummary(IoSummary<T> $$0, BiConsumer<T, JsonObject> $$1) {
      ...
      $$0.largestSizeContributors().forEach($$2x -> {
         ...
         $$1.accept($$4, $$3xx);
         $$3xx.addProperty("totalBytes", $$5.totalSize());
         $$3xx.addProperty("count", $$5.totalCount());
         $$3xx.addProperty("averageSize", $$5.averageSize());
      });
      return $$2;
   }
   ...
   private JsonElement cpu(List<CpuLoadStat> $$0) {
      ...
      BiFunction<List<CpuLoadStat>, ToDoubleFunction<CpuLoadStat>, JsonObject> $$2 = ($$0x, $$1x) -> {
         ...
         $$2xx.addProperty("min", $$3.getMin());
         $$2xx.addProperty("average", $$3.getAverage());
         $$2xx.addProperty("max", $$3.getMax());
         return $$2xx;
      };
