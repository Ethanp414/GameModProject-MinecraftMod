...
public class FurnaceRecipeFix extends DataFix {
   ...
   private <R> Typed<?> updateFurnaceContents(Type<R> $$0, Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> $$1, Typed<?> $$2) {
      Dynamic<?> $$3 = $$2.getOrCreate(DSL.remainderFinder());
      int $$4 = $$3.get("RecipesUsedSize").asInt(0);
      $$3 = $$3.remove("RecipesUsedSize");
      List<Pair<R, Integer>> $$5 = Lists.<Pair<R, Integer>>newArrayList();
      ...
      for(int $$6 = 0; $$6 < $$4; ++$$6) {
         ...
         if ($$10 > 0) {
            $$9.ifPresent($$3x -> {
               Optional<? extends Pair<R, ? extends Dynamic<?>>> $$4xx = $$0.read($$3x).result();
               $$4xx.ifPresent($$2xx -> $$5.add(Pair.of($$2xx.getFirst(), $$10)));
            });
         }
