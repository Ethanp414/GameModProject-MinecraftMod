...
public class EntityEquipmentToArmorAndHandFix extends DataFix {
   ...
   private <ItemStackOld, ItemStackNew> TypeRewriteRule cap(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1) {
      Type<Pair<String, Either<List<ItemStackOld>, Unit>>> $$2 = DSL.named(
      ...
      if (!$$2.equals(this.getInputSchema().getType(References.ENTITY_EQUIPMENT))) {
         ...
         return TypeRewriteRule.seq(
            ...
            this.fixTypeEverywhere(
               ...
               $$3,
               $$1x -> {
                  ItemStackNew $$2xx = (ItemStackNew)((Pair)$$1.read(new Dynamic($$1x).emptyMap())
                        .result()
                        .orElseThrow(() -> new IllegalStateException("Could not parse newly created empty itemstack.")))
                     .getFirst();
                  Either<ItemStackNew, Unit> $$3xx = Either.right(DSL.unit());
                  return $$2xx -> $$2xx.mapSecond($$2xxx -> {
                        List<ItemStackOld> $$3xxx = $$2xxx.map(Function.identity(), $$0xxxx -> List.of());
                        Either<List<ItemStackNew>, Unit> $$4 = Either.right(DSL.unit());
                        Either<List<ItemStackNew>, Unit> $$5 = Either.right(DSL.unit());
                        if (!$$3xxx.isEmpty()) {
                           $$4 = Either.left(Lists.<Object>newArrayList($$3xxx.getFirst(), $$2x));
                        }
      
                        if ($$3xxx.size() > 1) {
                           List<ItemStackNew> $$6 = Lists.<ItemStackNew>newArrayList($$2x, $$2x, $$2x, $$2x);
      
                           for(int $$7 = 1; $$7 < Math.min($$3xxx.size(), 5); ++$$7) {
                              $$6.set($$7 - 1, $$3xxx.get($$7));
                           }
   ...
   private static Dynamic<?> fixDropChances(Dynamic<?> $$0) {
      ...
      if ($$1.isPresent()) {
         Iterator<Float> $$2 = Stream.concat(((Stream)$$1.get()).map($$0x -> $$0x.asFloat(0.0F)), Stream.generate(() -> 0.0F)).iterator();
         float $$3 = $$2.next();
         ...
         if ($$0.get("ArmorDropChances").result().isEmpty()) {
            $$0 = $$0.set(
               "ArmorDropChances", $$0.createList(Stream.of((Float)$$2.next(), (Float)$$2.next(), (Float)$$2.next(), (Float)$$2.next()).map($$0::createFloat))
            );
         }
      }
