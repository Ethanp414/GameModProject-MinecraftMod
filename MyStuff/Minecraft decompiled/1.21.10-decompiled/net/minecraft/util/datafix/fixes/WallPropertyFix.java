...
public class WallPropertyFix extends DataFix {
   ...
   private static <T> Dynamic<T> fixWallProperty(Dynamic<T> $$0, String $$1) {
      return $$0.update($$1, $$0x -> DataFixUtils.orElse($$0x.asString().result().map(WallPropertyFix::mapProperty).map($$0x::createString), $$0x));
   }
   ...
   private static <T> Dynamic<T> upgradeBlockStateTag(Dynamic<T> $$0) {
      ...
      return !$$1 ? $$0 : $$0.update("Properties", $$0x -> {
         Dynamic<?> $$1xx = fixWallProperty($$0x, "east");
         $$1xx = fixWallProperty($$1xx, "west");
         $$1xx = fixWallProperty($$1xx, "north");
         return fixWallProperty($$1xx, "south");
      });
   }
}
