...
public class AttributeModifierIdFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(
         this.fixTypeEverywhereTyped(
            ...
            $$0,
            $$1x -> $$1x.updateTyped($$1, $$0xx -> $$0xx.update(DSL.remainderFinder(), AttributeModifierIdFix::fixItemStackComponents))
         ),
   ...
   private static Dynamic<?> convertModifierForEntity(Dynamic<?> $$0) {
   ...
   private static Dynamic<?> fixItemStackComponents(Dynamic<?> $$0) {
      return $$0.update(
         ...
         $$0x -> $$0x.update(
               "modifiers",
               $$0xx -> DataFixUtils.orElse($$0xx.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map($$0xx::createList), $$0xx)
            )
      );
   }
   ...
   private static Dynamic<?> fixAttribute(Dynamic<?> $$0) {
      return $$0.renameField("Name", "id")
         ...
         .renameAndFixField(
            "Modifiers",
            "modifiers",
            $$1 -> DataFixUtils.orElse(
                  $$1.asStreamOpt()
                     .result()
                     .map($$0xx -> $$0xx.map(AttributeModifierIdFix::convertModifierForEntity))
   ...
   private static Typed<?> fixEntity(Typed<?> $$0) {
      return $$0.update(
         ...
         $$0x -> $$0x.renameAndFixField(
               "Attributes",
               "attributes",
               $$0xx -> DataFixUtils.orElse(
                     $$0xx.asStreamOpt().result().map($$0xxx -> $$0xxx.map(AttributeModifierIdFix::fixAttribute)).map($$0xx::createList), $$0xx
                  )
            )
      );
   }
