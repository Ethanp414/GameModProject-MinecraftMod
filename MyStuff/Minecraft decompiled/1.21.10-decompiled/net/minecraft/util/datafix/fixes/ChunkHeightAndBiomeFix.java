...
public class ChunkHeightAndBiomeFix extends DataFix {
      ...
      return this.fixTypeEverywhereTyped(
         ...
         $$4x -> $$4x.updateTyped(
               ...
               $$3xx -> {
                  ...
                  OptionalDynamic<?> $$5xx = $$4x.get(DSL.remainderFinder()).get("__context");
                  String $$6xx = (String)$$5xx.get("dimension").asString().result().orElse("");
                  String $$7 = (String)$$5xx.get("generator").asString().result().orElse("");
                  boolean $$8 = "minecraft:overworld".equals($$6xx);
                  MutableBoolean $$9 = new MutableBoolean();
                  int $$10 = $$8 ? -4 : 0;
                  Dynamic<?>[] $$11 = getBiomeContainers($$4xxx, $$8, $$10, $$9);
                  Dynamic<?> $$12 = makePalettedContainer(
                     $$4xxx.createList(Stream.of($$4xxx.createMap(ImmutableMap.of($$4xxx.createString("Name"), $$4xxx.createString("minecraft:air")))))
                  );
                  Set<String> $$13 = Sets.newHashSet();
                  MutableObject<Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer>> $$14 = new MutableObject((Supplier)() -> null);
                  $$3xx = $$3xx.updateTyped($$2, $$6, $$7x -> {
                     IntSet $$8xx = new IntOpenHashSet();
                     Dynamic<?> $$9xx = (Dynamic)$$7x.write().result().orElseThrow(() -> new IllegalStateException("Malformed Chunk.Level.Sections"));
                     List<Dynamic<?>> $$10xx = (List)$$9xx.asStream().map($$6xxx -> {
                        int $$7xxx = $$6xxx.get("Y").asInt(0);
                        Dynamic<?> $$8xxx = DataFixUtils.orElse($$6xxx.get("Palette").result().flatMap($$2xxxxx -> {
                           $$2xxxxx.asStream().map($$0xxxxxx -> $$0xxxxxx.get("Name").asString("minecraft:air")).forEach($$13::add);
                           return $$6xxx.get("BlockStates").result().map($$1xxxxxx -> makeOptimizedPalettedContainer($$2xxxxx, $$1xxxxxx));
                        }), $$12);
                        Dynamic<?> $$9xxx = $$6xxx;
                        int $$10xxx = $$7xxx - $$10;
                        if ($$10xxx >= 0 && $$10xxx < $$11.length) {
                           $$9xxx = $$6xxx.set("biomes", $$11[$$10xxx]);
                        }
      
                        $$8x.add($$7xxx);
                        if ($$6xxx.get("Y").asInt(Integer.MAX_VALUE) == 0) {
                           $$14.setValue((Supplier)() -> {
                              List<? extends Dynamic<?>> $$1xxxxxx = $$8xx.get("palette").asList(Function.identity());
                              long[] $$2xxxxxx = $$8xx.get("data").asLongStream().toArray();
                              return new ChunkProtoTickListFix.PoorMansPalettedContainer($$1xxxxxx, $$2xxxxxx);
                           });
                        }
      
                        return $$9xxx.set("block_states", $$8xxx).remove("Palette").remove("BlockStates");
                     }).collect(Collectors.toCollection(ArrayList::new));
      
                     for(int $$11xx = 0; $$11xx < $$11.length; ++$$11xx) {
                        int $$12xx = $$11xx + $$10;
                        if ($$8xx.add($$12xx)) {
                           Dynamic<?> $$13xx = $$4xx.createMap(Map.of($$4xx.createString("Y"), $$4xx.createInt($$12xx)));
                           $$13xx = $$13xx.set("block_states", $$12);
                           $$13xx = $$13xx.set("biomes", $$11[$$11xx]);
                           $$10xx.add($$13xx);
                        }
                     }
      
                     return Util.readTypedOrThrow($$6, $$4xx.createList($$10xx.stream()));
                  });
                  return $$3xx.update(
                     DSL.remainderFinder(),
                     $$5xx -> {
                        if ($$8) {
                           $$5xx = this.predictChunkStatusBeforeSurface($$5xx, $$13);
                        }
                        ...
                        return updateChunkTag(
                           ...
                           $$8,
                           $$9.booleanValue(),
                           "minecraft:noise".equals($$7),
                           (Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer>)$$14.getValue()
                        );
                     }
                  );
               }
   ...
   private static Dynamic<?>[] getBiomeContainers(Dynamic<?> $$0, boolean $$1, int $$2, MutableBoolean $$3) {
      ...
      if ($$5 != null && $$5.length == 1536) {
         ...
         for(int $$6 = 0; $$6 < 24; ++$$6) {
            $$4[$$6] = makeBiomeContainer($$0, $$2x -> getOldBiome($$5, $$6 * 64 + $$2x));
         }
      } else if ($$5 != null && $$5.length == 1024) {
         for(int $$8 = 0; $$8 < 16; ++$$8) {
            int $$9 = $$8 - $$2;
            $$4[$$9] = makeBiomeContainer($$0, $$2x -> getOldBiome($$5, $$8 * 64 + $$2x));
         }

         if ($$1) {
            Dynamic<?> $$11 = makeBiomeContainer($$0, $$1x -> getOldBiome($$5, $$1x % 16));
            Dynamic<?> $$12 = makeBiomeContainer($$0, $$1x -> getOldBiome($$5, $$1x % 16 + 1008));
         ...
         Arrays.fill($$4, makePalettedContainer($$0.createList(Stream.of($$0.createString("minecraft:plains")))));
      }
   ...
   private static Dynamic<?> updateChunkTag(
      Dynamic<?> $$0, boolean $$1, boolean $$2, boolean $$3, Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> $$4
   ) {
      $$0 = $$0.remove("Biomes");
      if (!$$1) {
         return updateCarvingMasks($$0, 16, 0);
      } else if ($$2) {
         return updateCarvingMasks($$0, 24, 0);
      } else {
         $$0 = updateHeightmaps($$0);
         $$0 = addPaddingEntries($$0, "LiquidsToBeTicked");
         $$0 = addPaddingEntries($$0, "PostProcessing");
         $$0 = addPaddingEntries($$0, "ToBeTicked");
         $$0 = updateCarvingMasks($$0, 24, 4);
         ...
         if (!$$3) {
            ...
            if ($$5.isPresent()) {
               ...
               if (!"empty".equals($$7)) {
                  $$0 = $$0.set(
                     "blending_data", $$0.createMap(ImmutableMap.of($$0.createString("old_noise"), $$0.createBoolean(STATUS_IS_OR_AFTER_NOISE.contains($$7))))
                  );
                  if (!SharedConstants.DEBUG_DISABLE_BELOW_ZERO_RETROGENERATION) {
                     ChunkProtoTickListFix.PoorMansPalettedContainer $$8 = (ChunkProtoTickListFix.PoorMansPalettedContainer)$$4.get();
                     if ($$8 != null) {
   ...
   private static Dynamic<?> updateHeightmaps(Dynamic<?> $$0) {
      return $$0.update("Heightmaps", $$0x -> {
         for(String $$1 : HEIGHTMAP_TYPES) {
            $$0x = $$0x.update($$1, ChunkHeightAndBiomeFix::getFixedHeightmap);
         }
   ...
   private static Dynamic<?> makeBiomeContainer(Dynamic<?> $$0, Int2IntFunction $$1) {
      ...
      Dynamic<?> $$5 = $$0.createList($$2.keySet().stream().map($$1x -> $$0.createString((String)BIOMES_BY_ID.getOrDefault($$1x, "minecraft:plains"))));
      int $$6 = ceillog2($$2.size());
      if ($$6 == 0) {
   ...
   private static Dynamic<?> makeOptimizedPalettedContainer(Dynamic<?> $$0, Dynamic<?> $$1) {
      ...
      if ($$2.size() == 1) {
         ...
         $$0 = padPaletteEntries($$0, $$1, $$2);
         return makePalettedContainer($$0, $$1);
      }
   }
