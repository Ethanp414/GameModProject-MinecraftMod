...
public class BannerPatternFormatFix extends NamedEntityFix {
   ...
   private static Dynamic<?> fixTag(Dynamic<?> $$0) {
      return $$0.renameAndFixField("Patterns", "patterns", $$0x -> $$0x.createList($$0x.asStream().map(BannerPatternFormatFix::fixLayer)));
   }
   ...
   private static Dynamic<?> fixLayer(Dynamic<?> $$0) {
      $$0 = $$0.renameAndFixField(
         "Pattern",
         "pattern",
         $$0x -> DataFixUtils.orElse($$0x.asString().map($$0xx -> (String)PATTERN_ID_MAP.getOrDefault($$0xx, $$0xx)).map($$0x::createString).result(), $$0x)
      );
