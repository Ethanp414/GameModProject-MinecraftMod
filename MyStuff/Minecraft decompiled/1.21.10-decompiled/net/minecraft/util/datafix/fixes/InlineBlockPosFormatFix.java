   ...
   private Dynamic<?> fixPlayer(Dynamic<?> $$0) {
      ...
      if ($$1.isPresent() && $$2.isPresent() && $$3.isPresent()) {
         Dynamic<?> $$4 = $$0.createMap(
            Map.of(
               $$0.createString("pos"),
               ExtraDataFixUtils.createBlockPos($$0, ((Number)$$1.get()).intValue(), ((Number)$$2.get()).intValue(), ((Number)$$3.get()).intValue())
            )
         );
      ...
      if ($$5.isPresent()) {
         $$0 = $$0.remove("enteredNetherPosition")
            .set(
               ...
               $$0.createList(
                  Stream.of(
                     $$0.createDouble(((Dynamic)$$5.get()).get("x").asDouble(0.0)),
                     $$0.createDouble(((Dynamic)$$5.get()).get("y").asDouble(0.0)),
                     $$0.createDouble(((Dynamic)$$5.get()).get("z").asDouble(0.0))
                  )
               )
            );
      }
