...
public class MobEffectIdFix extends DataFix {
   private static final Int2ObjectMap<String> ID_MAP = Util.make(new Int2ObjectOpenHashMap(), $$0 -> {
   ...
   private static <T> Optional<Dynamic<T>> getAndConvertMobEffectId(Dynamic<T> $$0, String $$1) {
      return $$0.get($$1).asNumber().result().map($$0x -> (String)ID_MAP.get($$0x.intValue())).map($$0::createString);
   }
   ...
   private static <T> Dynamic<T> updateMobEffectInstance(Dynamic<T> $$0) {
      $$0 = updateMobEffectIdField($$0, "Id", "id");
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(MobEffectIdFix::updateMobEffectInstance);
   ...
   private static <T> Dynamic<T> updateMobEffectInstanceList(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(MobEffectIdFix::updateMobEffectInstance)));
      return $$0.replaceField($$1, $$2, $$3);
   }

   private static <T> Dynamic<T> updateSuspiciousStewEntry(Dynamic<T> $$0, Dynamic<T> $$1) {
      $$1 = updateMobEffectIdField($$0, "EffectId", $$1, "id");
   ...
   private Typed<?> updateNamedChoice(Typed<?> $$0, TypeReference $$1, String $$2, Function<Dynamic<?>, Dynamic<?>> $$3) {
   ...
   private TypeRewriteRule blockEntityFixer() {
      ...
      return this.fixTypeEverywhereTyped(
         "BlockEntityMobEffectIdFix", $$0, $$0x -> this.updateNamedChoice($$0x, References.BLOCK_ENTITY, "minecraft:beacon", $$0xx -> {
               $$0xx = updateMobEffectIdField($$0xx, "Primary", "primary_effect");
               return updateMobEffectIdField($$0xx, "Secondary", "secondary_effect");
            })
      );
   }
   ...
   private TypeRewriteRule entityFixer() {
      ...
      return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", $$0, $$0x -> {
         $$0x = this.updateNamedChoice($$0x, References.ENTITY, "minecraft:mooshroom", MobEffectIdFix::fixMooshroomTag);
         $$0x = this.updateNamedChoice($$0x, References.ENTITY, "minecraft:arrow", MobEffectIdFix::fixArrowTag);
         $$0x = this.updateNamedChoice($$0x, References.ENTITY, "minecraft:area_effect_cloud", MobEffectIdFix::fixAreaEffectCloudTag);
         return $$0x.update(DSL.remainderFinder(), MobEffectIdFix::updateLivingEntityTag);
      });
   }

   private TypeRewriteRule playerFixer() {
      Type<?> $$0 = this.getInputSchema().getType(References.PLAYER);
      return this.fixTypeEverywhereTyped("PlayerMobEffectIdFix", $$0, $$0x -> $$0x.update(DSL.remainderFinder(), MobEffectIdFix::updateLivingEntityTag));
   }

   private static <T> Dynamic<T> fixSuspiciousStewTag(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(MobEffectIdFix::updateSuspiciousStewEntry)));
         ...
         $$2x -> {
            ...
            if ($$3.isPresent()) {
               ...
               if ($$4.equals("minecraft:suspicious_stew")) {
                  return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), MobEffectIdFix::fixSuspiciousStewTag));
               }
   
               if (MOB_EFFECT_INSTANCE_CARRIER_ITEMS.contains($$4)) {
                  return $$2x.updateTyped(
                     $$2,
                     $$0xx -> $$0xx.update(
                           DSL.remainderFinder(), $$0xxx -> updateMobEffectInstanceList($$0xxx, "CustomPotionEffects", "custom_potion_effects")
                        )
                  );
               }
            }
