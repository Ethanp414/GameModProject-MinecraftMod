...
public class AttributesRenameLegacy extends DataFix {
   ...
   private Dynamic<?> fixName(Dynamic<?> $$0) {
      return DataFixUtils.orElse($$0.asString().result().map(this.renames).map($$0::createString), $$0);
   }
      ...
      return $$0.update(
         ...
         $$0x -> $$0x.update(
               ...
               $$0xx -> DataFixUtils.orElse(
                     $$0xx.asStreamOpt().result().map($$0xxx -> $$0xxx.map($$0xxxx -> $$0xxxx.update("AttributeName", this::fixName))).map($$0xx::createList),
                     $$0xx
                  )
            )
      );
