...
public class SaddleEquipmentSlotFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped(
         ...
         $$3,
         $$3x -> {
            String $$4xx = (String)$$3x.getOptional($$1).map(Pair::getFirst).map(NamespacedSchema::ensureNamespaced).orElse("");
            Typed<?> $$5 = ExtraDataFixUtils.cast($$4, $$3x);
            if (ENTITIES_WITH_SADDLE_ITEM.contains($$4xx)) {
               return Util.writeAndReadTypedOrThrow($$5, $$3, SaddleEquipmentSlotFix::fixEntityWithSaddleItem);
            } else {
               return ENTITIES_WITH_SADDLE_FLAG.contains($$4xx)
                  ? Util.writeAndReadTypedOrThrow($$5, $$3, SaddleEquipmentSlotFix::fixEntityWithSaddleFlag)
