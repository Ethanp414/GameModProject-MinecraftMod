...
public abstract class EntityRenameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function<String, Type<?>> $$2 = Util.memoize((Function)($$2x -> {
         Type<?> $$3 = (Type)$$0.types().get($$2x);
         return ExtraDataFixUtils.patchSubType($$3, $$0, $$1);
      }));
      return this.fixTypeEverywhere(this.name, $$0, $$1, $$2x -> $$3 -> {
            ...
            if (!$$7.equals($$6.getSecond().getType(), true, true)) {
               throw new IllegalStateException(String.format(Locale.ROOT, "Dynamic type check failed: %s not equal to %s", $$7, $$6.getSecond().getType()));
            } else {
               return Pair.of((String)$$6.getFirst(), $$6.getSecond().getValue());
            }
         });
   }

   private <A> Typed<A> getEntity(Object $$0, DynamicOps<?> $$1, Type<A> $$2) {
      return new Typed<>($$2, $$1, (A)$$0);
   }
