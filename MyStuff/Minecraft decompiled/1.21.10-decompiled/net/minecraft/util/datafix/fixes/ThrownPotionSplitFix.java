...
public class ThrownPotionSplitFix extends EntityRenameFix {
   private final Supplier<ThrownPotionSplitFix.ItemIdFinder> itemIdFinder = Suppliers.memoize(() -> {
      Type<?> $$0xx = this.getInputSchema().getChoiceType(References.ENTITY, "minecraft:potion");
      Type<?> $$1 = ExtraDataFixUtils.patchSubType($$0xx, this.getInputSchema().getType(References.ENTITY), this.getOutputSchema().getType(References.ENTITY));
   ...
   protected Pair<String, Typed<?>> fix(String $$0, Typed<?> $$1) {
      if (!$$0.equals("minecraft:potion")) {
         ...
         String $$2 = ((ThrownPotionSplitFix.ItemIdFinder)this.itemIdFinder.get()).getItemId($$1);
   ...
   static record ItemIdFinder(OpticFinder<?> itemFinder, OpticFinder<Pair<String, String>> itemIdFinder) {
      public String getItemId(Typed<?> $$0) {
         return (String)$$0.getOptionalTyped(this.itemFinder)
