...
public class DebugSubscription<T> {
   ...
   public static record Event<T>(DebugSubscription<T> subscription, T value) {
      ...
      private static <T> StreamCodec<? super RegistryFriendlyByteBuf, DebugSubscription.Event<T>> streamCodec(DebugSubscription<T> $$0) {
         return ((StreamCodec)Objects.requireNonNull($$0.valueStreamCodec))
            .map($$1 -> new DebugSubscription.Event<>($$0, (T)$$1), DebugSubscription.Event::value);
      }
   }
   ...
   public static record Update<T>(DebugSubscription<T> subscription, Optional<T> value) {
      ...
      private static <T> StreamCodec<? super RegistryFriendlyByteBuf, DebugSubscription.Update<T>> streamCodec(DebugSubscription<T> $$0) {
         return ByteBufCodecs.optional((StreamCodec)Objects.requireNonNull($$0.valueStreamCodec))
