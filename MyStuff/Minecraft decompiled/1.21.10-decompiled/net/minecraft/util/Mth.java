...
public class Mth {
   ...
   private static final float[] SIN = Util.make(new float[65536], $$0x -> {
      for(int $$1xx = 0; $$1xx < $$0x.length; ++$$1xx) {
         $$0x[$$1xx] = (float)Math.sin((double)$$1xx * Math.PI * 2.0 / 65536.0);
      }
   });
   ...
   public static float sin(float $$0) {
      return SIN[(int)($$0 * 10430.378F) & 65535];
   }

   public static float cos(float $$0) {
      return SIN[(int)($$0 * 10430.378F + 16384.0F) & 65535];
   }
   ...
   public static int smallestSquareSide(int $$0) {
      if ($$0 < 0) {
         ...
         return ceil(Math.sqrt((double)$$0));
      }
   }
   ...
   public static int hsvToArgb(float $$0, float $$1, float $$2, int $$3) {
      ...
      return ARGB.color($$3, clamp((int)($$9 * 255.0F), 0, 255), clamp((int)($$10 * 255.0F), 0, 255), clamp((int)($$11 * 255.0F), 0, 255));
   }
   ...
   public static int lerpDiscrete(float $$0, int $$1, int $$2) {
      int $$3 = $$2 - $$1;
      return $$1 + floor($$0 * (float)($$3 - 1)) + ($$0 > 0.0F ? 1 : 0);
   }
   ...
   public static double wobble(double $$0) {
      return $$0 + (2.0 * RandomSource.create((long)floor($$0 * 3000.0)).nextDouble() - 1.0) * 1.0E-7 / 2.0;
   }
   ...
   public static float length(float $$0, float $$1) {
      return (float)Math.sqrt(lengthSquared((double)$$0, (double)$$1));
   }
   ...
   public static IntStream outFromOrigin(int $$0, int $$1, int $$2, int $$3) {
      if ($$1 > $$2) {
         ...
         return IntStream.iterate($$4, $$3x -> {
            int $$4xx = Math.abs($$4 - $$3x);
            return $$4 - $$4xx >= $$1 || $$4 + $$4xx <= $$2;
         }, $$4x -> {
            boolean $$5 = $$4x <= $$4;
            int $$6 = Math.abs($$4 - $$4x);
            boolean $$7 = $$4 + $$6 + $$3 <= $$2;
   ...
   public static float easeInOutSine(float $$0) {
      return -(cos((float) Math.PI * $$0) - 1.0F) / 2.0F;
   }
