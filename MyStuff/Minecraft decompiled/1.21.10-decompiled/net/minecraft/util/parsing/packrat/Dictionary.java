...
public class Dictionary<S> {
   ...
   public <T> NamedRule<S, T> put(Atom<T> $$0, Rule<S, T> $$1) {
      Dictionary.Entry<S, T> $$2 = (Dictionary.Entry)this.terms.computeIfAbsent($$0, Dictionary.Entry::new);
      if ($$2.value != null) {
   ...
   public void checkAllBound() {
      List<? extends Atom<?>> $$0 = this.terms.entrySet().stream().filter($$0x -> $$0x.getValue() == null).map(java.util.Map.Entry::getKey).toList();
      if (!$$0.isEmpty()) {
   ...
   public <T> NamedRule<S, T> getOrThrow(Atom<T> $$0) {
      return (NamedRule<S, T>)Objects.requireNonNull((Dictionary.Entry)this.terms.get($$0), () -> "No rule called " + $$0);
   }
