...
public class EventLogDirectory {
   ...
   public EventLogDirectory.FileList listFiles() throws IOException {
      ...
      try {
         var2 = new EventLogDirectory.FileList(
            $$0.filter($$0x -> Files.isRegularFile($$0x, new LinkOption[0])).map(this::parseFile).filter(Objects::nonNull).toList()
         );
   ...
   private static void writeCompressed(ReadableByteChannel $$0, Path $$1) throws IOException {
      OutputStream $$2 = new GZIPOutputStream(Files.newOutputStream($$1));
   ...
   public static record CompressedFile(Path path, EventLogDirectory.FileId id) implements EventLogDirectory.File {
      ...
      public Reader openReader() throws IOException {
         return !Files.exists(this.path, new LinkOption[0])
            ? null
            : new BufferedReader(new InputStreamReader(new GZIPInputStream(Files.newInputStream(this.path))));
      }
   ...
   public static class FileList implements Iterable<EventLogDirectory.File> {
      ...
      public EventLogDirectory.FileList compressAll() {
         ...
         while($$0.hasNext()) {
            EventLogDirectory.File $$1 = (EventLogDirectory.File)$$0.next();
