...
public class ModelWarpedToad extends AdvancedEntityModel<EntityWarpedToad> {
   ...
   public Iterable<AdvancedModelBox> getAllParts() {
      return ImmutableList.of(
         ...
         this.left_arm,
         this.left_leg,
         new AdvancedModelBox[]{this.left_foot, this.left_gland, this.tongue, this.right_foot_pivot, this.left_foot_pivot}
      );
   }
   ...
   public void setupAnim(EntityWarpedToad entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float glowyBob = 0.75F + (Mth.m_14089_(ageInTicks * 0.2F) + 1.0F) * 0.125F + attackProgress * 0.1F;
      ...
      this.left_arm.rotationPointZ = this.left_arm.rotationPointZ
         + 1.5F * (float)(Math.sin(limbSwing * walkSpeed - 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
      this.left_arm.rotationPointY = this.left_arm.rotationPointY
         + 0.5F * (float)(Math.sin(limbSwing * walkSpeed - 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
      this.right_arm.rotationPointZ = this.right_arm.rotationPointZ
         + 1.5F * (float)(Math.sin(-(limbSwing * walkSpeed) + 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
      this.right_arm.rotationPointY = this.right_arm.rotationPointY
         + 0.5F * (float)(Math.sin(-(limbSwing * walkSpeed) + 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
      float leftLegS = (float)(Math.sin(limbSwing * walkSpeed - 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
      float rightLegs = (float)(Math.sin(-(limbSwing * walkSpeed) + 2.5) * walkSwingAmount * walkDegree - walkSwingAmount * walkDegree);
