...
public class ModelRainFrog extends AdvancedEntityModel<EntityRainFrog> {
   ...
   public void setupAnim(EntityRainFrog entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float digAmount = Mth.m_14036_((float)Math.sin(burrowProgress * Math.PI / 5.0), 0.0F, 1.0F);
      ...
      float leftLegS = (float)(Math.sin(limbSwing * walkSpeed - 2.5) * limbSwingAmount * walkDegree - limbSwingAmount * walkDegree);
      float rightLegS = (float)(Math.sin(-(limbSwing * walkSpeed) + 2.5) * limbSwingAmount * walkDegree - limbSwingAmount * walkDegree);
