...
public class ModelEnderiophage extends AdvancedEntityModel<EntityEnderiophage> {
   ...
   public Iterable<AdvancedModelBox> getAllParts() {
      return ImmutableList.of(
         ...
         this.tailback_right,
         this.tailfront_left,
         this.tailfront_right,
         this.tailmid_left,
         this.tailmid_right,
         this.tailback_leftPivot,
         this.tailback_rightPivot,
         this.tailfront_leftPivot,
         this.tailfront_rightPivot,
         this.tailmid_leftPivot,
         new AdvancedModelBox[]{this.tailmid_rightPivot, this.body, this.capsid, this.eye, this.mouth, this.sheath, this.collar}
      );
   }
   ...
   public void setupAnim(EntityEnderiophage entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float phagePitch = Maths.rad(Mth.m_14189_(partialTicks, entity.prevPhagePitch, entity.getPhagePitch()));
      float totalYaw = Maths.rad(Mth.m_14189_(partialTicks, entity.f_20884_, entity.f_20883_));
      ...
      if (entity.m_20159_()) {
         ...
         this.body.rotateAngleY = this.body.rotateAngleY + (float) (Math.PI / 2) * entity.passengerIndex;
         this.sheath.setScale(1.0F, (float)(0.85F + Math.sin(ageInTicks) * 0.15F), 1.0F);
         this.collar.rotationPointY = this.collar.rotationPointY - (float)(Math.sin(ageInTicks) * 0.15F - 0.15F) * 12.0F;
         this.capsid
            .setScale(
               (float)(0.85F + Math.sin(ageInTicks + 2.0F) * 0.15F),
               (float)(1.0 + Math.sin(ageInTicks) * 0.15F),
               (float)(0.85F + Math.sin(ageInTicks + 2.0F) * 0.15F)
            );
         this.mouth.rotationPointY = (float)(this.mouth.rotationPointY + (Math.sin(ageInTicks) + 1.0) * 2.0);
      ...
      if (look != null) {
         ...
         float f = Mth.m_14116_((float)(vector3d2.f_82479_ * vector3d2.f_82479_ + vector3d2.f_82481_ * vector3d2.f_82481_)) - totalYaw;
         this.eye.rotateAngleY = this.eye.rotateAngleY + (-((float)Mth.m_14136_(vector3d2.f_82479_, vector3d2.f_82481_)) - totalYaw);
         this.eye.rotateAngleX = (float)(
            this.eye.rotateAngleX + (-Mth.m_14008_(vector3d2.f_82480_ * 0.5, -Math.PI / 2, Math.PI / 2) + phagePitch * flyProgress * 0.2F)
         );
      }
   }
