...
public class ModelBananaSlug extends AdvancedEntityModel<EntityBananaSlug> {
   ...
   public void setupAnim(EntityBananaSlug entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float antennaBack = -0.5F + (float)Math.sin(ageInTicks * idleSpeed + 3.0) * 0.2F;
      ...
      float stretch2 = (float)(Math.sin(limbSwing * -walkSpeed + 1.0F) * limbSwingAmount) + limbSwingAmount;
      ...
      float slimeYaw = Mth.m_14177_(entity.prevTrailYaw + (entity.trailYaw - entity.prevTrailYaw) * partialTick - yaw) * 0.65F;
      this.goo.rotationPointX = Mth.m_14031_(limbSwing * -walkSpeed - 1.0F) * limbSwingAmount;
      this.goo.rotateAngleY = this.goo.rotateAngleY + Maths.rad(slimeYaw);
      this.tail.rotateAngleY = this.tail.rotateAngleY + Maths.rad(slimeYaw * 0.8F);
