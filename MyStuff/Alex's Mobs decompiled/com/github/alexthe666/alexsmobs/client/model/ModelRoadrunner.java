...
public class ModelRoadrunner extends AdvancedEntityModel<EntityRoadrunner> {
   ...
   public void setupAnim(EntityRoadrunner entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netneckYaw, float neckPitch) {
      ...
      this.left_leg.rotationPointY = this.left_leg.rotationPointY
         + 1.5F * (float)(Math.sin(limbSwing * walkSpeed + 2.0) * limbSwingAmount * walkDegree - limbSwingAmount * walkDegree);
      this.right_leg.rotationPointY = this.right_leg.rotationPointY
         + 1.5F * (float)(Math.sin(-(limbSwing * walkSpeed) - 2.0) * limbSwingAmount * walkDegree - limbSwingAmount * walkDegree);
      float partialTicks = Minecraft.m_91087_().m_91296_();
      float f = Mth.m_14179_(partialTicks, entityIn.oFlap, entityIn.wingRotation);
      float f1 = Mth.m_14179_(partialTicks, entityIn.oFlapSpeed, entityIn.destPos);
      float wingSwing = (Mth.m_14031_(f) + 1.0F) * f1;
   ...
   public Iterable<AdvancedModelBox> getAllParts() {
      return ImmutableList.of(
         ...
         this.right_spin,
         this.left_spin,
         new AdvancedModelBox[]{this.beak, this.left_knee, this.right_knee, this.left_foot, this.right_foot}
      );
   }
