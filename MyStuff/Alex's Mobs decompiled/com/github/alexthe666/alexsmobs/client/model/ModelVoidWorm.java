...
public class ModelVoidWorm extends AdvancedEntityModel<EntityVoidWorm> {
   ...
   public void setupAnim(EntityVoidWorm entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      this.root.rotationPointY = this.root.rotationPointY + (-3.0F - Mth.m_14036_(headPitch * -0.125F, -10.0F, 10.0F) * 0.3F);
      this.root.rotationPointZ += -15.0F + limbSwingAmount * 10.0F;
      float yawAmount = (entityIn.prevWormAngle + (entityIn.getWormAngle() - entityIn.prevWormAngle) * (ageInTicks - entityIn.f_19797_))
         / (180.0F / (float)Math.PI)
         * 0.5F;
      this.neck.rotateAngleZ += yawAmount;
      float jawProgress = entityIn.prevJawProgress + (entityIn.jawProgress - entityIn.prevJawProgress) * (ageInTicks - entityIn.f_19797_);
   ...
   public Iterable<AdvancedModelBox> getAllParts() {
      return ImmutableList.of(
         ...
         this.neck,
         this.head,
         this.bottomfrills_left,
         this.bottomfrills_right,
         this.eye_bottom_r1,
         this.eye_top_r1,
         this.topfrills_left,
         this.topfrills_right,
         this.topjaw,
         this.bottomjaw
      );
   }
