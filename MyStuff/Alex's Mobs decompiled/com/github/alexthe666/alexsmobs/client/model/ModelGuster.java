...
public class ModelGuster extends AdvancedEntityModel<EntityGuster> {
   ...
   public void setupAnim(EntityGuster entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      if (entity.isGooglyEyes()) {
         this.eye_left.rotationPointY = this.eye_left.rotationPointY + (float)(Math.sin(ageInTicks * 0.7 - 2.0) * 1.9);
         this.eye_right.rotationPointY = this.eye_right.rotationPointY + (float)(Math.sin(ageInTicks * 0.7 + 2.0) * 1.9);
      } else {
         this.eye_left.rotationPointY = this.eye_left.rotationPointY + (float)(Math.sin(ageInTicks * 0.1 - 2.0) * 0.9);
         this.eye_right.rotationPointY = this.eye_right.rotationPointY + (float)(Math.sin(ageInTicks * 0.1 + 2.0) * 0.9);
      }
      ...
      this.tornado.rotationPointX = (float)(this.tornado.rotationPointX + Math.cos(ageInTicks * 0.7F) * 4.0);
      this.tornado.rotationPointZ = (float)(this.tornado.rotationPointZ + Math.sin(ageInTicks * 0.7F) * 4.0);
      this.tornado.rotationPointX = (float)(this.tornado.rotationPointX + (Math.cos(ageInTicks * 0.3F) * 2.0 - this.tornado.rotationPointX));
      this.tornado.rotationPointZ = (float)(this.tornado.rotationPointZ + (Math.sin(ageInTicks * 0.3F) * 2.0 - this.tornado.rotationPointZ));
      this.tornadomid.rotateAngleZ = (float)(this.tornadomid.rotateAngleZ + Math.sin(ageInTicks * 0.2F) * 0.1);
      ...
      this.tornado4.rotateAngleY = this.tornado4.rotateAngleY
         - (this.tornado.rotateAngleY + this.tornado2.rotateAngleY + this.tornado3.rotateAngleY + ageInTicks * 0.34F);
      this.tornadomid.rotateAngleY += ageInTicks * 0.5F;
      this.eyes.rotationPointZ = (float)(this.eyes.rotationPointZ - (2.0 + Math.cos(this.tornado3.rotateAngleY)));
   }
