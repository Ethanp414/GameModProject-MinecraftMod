...
public class ModelCosmaw extends AdvancedEntityModel<EntityCosmaw> {
   ...
   public Iterable<AdvancedModelBox> getAllParts() {
      return ImmutableList.of(
         ...
         this.leftFin,
         this.rightArm,
         new AdvancedModelBox[]{this.rightFin, this.mouth, this.mouthArm1, this.mouthArm2, this.lowerJaw, this.topJaw}
      );
   }
   ...
   public void setupAnim(EntityCosmaw entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float openProgress = Math.max(
         Math.max(entity.prevOpenProgress + (entity.openProgress - entity.prevOpenProgress) * partialTick, clutchProgress), biteProgress
      );
      float cosmawPitch = (float)(Math.toRadians(entity.getClampedCosmawPitch(partialTick)) * (5.0F - clutchProgress) * 0.2F);
      ...
      float eyeYaw = Mth.m_14036_(netHeadYaw, -40.0F, 40.0F) / (180.0F / (float)Math.PI);
