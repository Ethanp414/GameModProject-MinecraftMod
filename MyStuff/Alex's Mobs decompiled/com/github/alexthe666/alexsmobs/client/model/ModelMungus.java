...
public class ModelMungus extends AdvancedEntityModel<EntityMungus> {
   ...
   public void setupAnim(EntityMungus entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float glowyBob = swell * 0.22F + 0.95F + (Mth.m_14089_(ageInTicks * (0.1F + swell * 0.2F)) + 1.0F) * (0.05F + swell * 0.02F);
      ...
      if (targetPos == null) {
         ...
         if (look != null) {
            ...
            this.eye.rotationPointX = this.eye.rotationPointX + Mth.m_14116_((float)Math.abs(d1)) * 2.0F * (float)Math.signum(d1);
         }
      } else {
         Vec3 vector3d = Vec3.m_82512_(targetPos);
         Vec3 vector3d1 = entity.m_20299_(0.0F);
         double d0 = vector3d.f_82480_ - vector3d1.f_82480_;
         if (d0 > 0.0) {
            this.eye.rotationPointY = -11.0F;
