...
public class RenderBlueJay extends MobRenderer<EntityBlueJay, ModelBlueJay> {
   ...
   public RenderBlueJay(Context renderManagerIn) {
      super(renderManagerIn, new ModelBlueJay(), 0.2F);
      this.m_115326_(new RenderBlueJay.LayerShiny());
   }

   protected void scale(EntityBlueJay mob, PoseStack matrixStackIn, float partialTicks) {
      matrixStackIn.m_85841_(0.9F, 0.9F, 0.9F);
      if (mob.m_20159_() && mob.m_20202_() != null && mob.m_20202_() instanceof EntityRaccoon entityRaccoon) {
         EntityRenderer raccoonRenderer = Minecraft.m_91087_().m_91290_().m_114382_(entityRaccoon);
         if (raccoonRenderer instanceof LivingEntityRenderer && ((LivingEntityRenderer)raccoonRenderer).m_7200_() instanceof ModelRaccoon raccoonModel) {
            ...
            Vec3 vec = raccoonModel.getRidingPosition(new Vec3(0.0, 0.0, -0.1F + standProgress * 0.1F));
   ...
   class LayerShiny extends RenderLayer<EntityBlueJay, ModelBlueJay> {
      ...
      public void render(
         ...
         int packedLightIn,
         EntityBlueJay entitylivingbaseIn,
         float limbSwing,
         ...
         if (entitylivingbaseIn.getFeedTime() > 0) {
            VertexConsumer ivertexbuilder = bufferIn.m_6299_(RenderType.m_110473_(RenderBlueJay.TEXTURE_SHINY));
            float alpha = (float)(1.0 + Math.sin(ageInTicks * 0.3F)) * 0.1F + 0.8F;
            ((ModelBlueJay)this.m_117386_())
