...
public class RenderFarseer extends MobRenderer<EntityFarseer, ModelFarseer> {
      ...
      if (!MinecraftForge.EVENT_BUS.post(new Pre(entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn))) {
         ...
         if (rendertype != null) {
            ...
            this.renderFarseerModel(
               matrixStackIn, bufferIn, rendertype, partialTicks, packedLightIn, i, flag1 ? 0.15F : Mth.m_14036_(portalLevel, 0.0F, 1.0F), entityIn
            );
         }
         ...
         if (entityIn.getAnimation() == EntityFarseer.ANIMATION_EMERGE) {
            ...
            int portalTexture = Mth.m_14045_(entityIn.getPortalFrame(), 0, PORTAL_TEXTURES.length - 1);
            VertexConsumer portalStatic = AMRenderTypes.createMergedVertexConsumer(
               bufferIn.m_6299_(AMRenderTypes.STATIC_PORTAL), bufferIn.m_6299_(RenderType.m_110473_(PORTAL_TEXTURES[portalTexture]))
            );
         ...
         if (entityIn.hasLaser() && laserTarget != null && !laserTarget.m_213877_()) {
            ...
            double d0 = Mth.m_14139_(partialTicks, laserTarget.f_19854_, laserTarget.m_20185_())
               - Mth.m_14139_(partialTicks, entityIn.f_19854_, entityIn.m_20185_())
               - angryShake.f_82479_;
            double d1 = Mth.m_14139_(partialTicks, laserTarget.f_19855_, laserTarget.m_20186_())
               + laserTarget.m_20192_()
               - Mth.m_14139_(partialTicks, entityIn.f_19855_, entityIn.m_20186_())
               - angryShake.f_82480_
               - laserHeight;
            double d2 = Mth.m_14139_(partialTicks, laserTarget.f_19856_, laserTarget.m_20189_())
               - Mth.m_14139_(partialTicks, entityIn.f_19856_, entityIn.m_20189_())
               - angryShake.f_82481_;
            double d4 = Math.sqrt(d0 * d0 + d2 * d2);
            float laserY = (float)(Mth.m_14136_(d2, d0) * 180.0F / (float)Math.PI) - 90.0F;
            float laserX = (float)(-(Mth.m_14136_(d1, d4) * 180.0F / (float)Math.PI));
   ...
   private void renderFarseerModel(
      ...
      if (entityIn.hasLaser()) {
         VertexConsumer staticyInsides = AMRenderTypes.createMergedVertexConsumer(
            source.m_6299_(AMRenderTypes.STATIC_ENTITY), source.m_6299_(RenderType.m_110473_(TEXTURE_EYE))
         );
      ...
      if (hurt > 0.0F) {
         ...
         VertexConsumer staticyScars = AMRenderTypes.createMergedVertexConsumer(
            source.m_6299_(AMRenderTypes.STATIC_ENTITY), source.m_6299_(RenderType.m_110473_(TEXTURE_SCARS))
         );
      ...
      RenderType afterimage = RenderType.m_234338_(this.getTextureLocation(entityIn));
      Vec3 colorOffset = entityIn.getLatencyOffsetVec(10, partialTicks).m_82490_(-0.2F).m_82549_(entityIn.angryShakeVec.m_82490_(0.3F));
      ...
      float scale = (float)Mth.m_14008_(colorOffset.m_82553_() * 0.1F, 0.0, 1.0);
      float angryProgress = entityIn.prevAngryProgress + (entityIn.angryProgress - entityIn.prevAngryProgress) * partialTicks;
      float afterimageAlpha1 = defAlpha * Math.max(((float)Math.sin((entityIn.f_19797_ + partialTicks) * 0.2F) + 1.0F) * 0.3F, angryProgress * 0.2F);
      float afterimageAlpha2 = defAlpha * Math.max(((float)Math.cos((entityIn.f_19797_ + partialTicks) * 0.2F) + 1.0F) * 0.3F, angryProgress * 0.2F);
      matrixStackIn.m_85836_();
   ...
   private static void laserOriginVertex(VertexConsumer p_114220_, Matrix4f p_114221_, Matrix3f p_114092_, int p_114222_, float xOffset, float yOffset) {
      p_114220_.m_252986_(p_114221_, 0.0F, 0.0F, 0.0F)
         .m_6122_(255, 255, 255, 255)
         .m_7421_(xOffset + 0.5F, yOffset)
   ...
   private static void laserLeftCornerVertex(
      ...
      p_114215_.m_252986_(p_114216_, -HALF_SQRT_3 * p_114218_, p_114217_, 0.0F)
         .m_6122_(255, 255, 255, 0)
         .m_7421_(xOffset, yOffset + 1.0F)
         .m_86008_(OverlayTexture.f_118083_)
         .m_85969_(240)
         .m_252939_(p_114092_, 0.0F, -1.0F, 0.0F)
         .m_5752_();
   ...
   private static void laserRightCornerVertex(
      ...
      p_114224_.m_252986_(p_114225_, HALF_SQRT_3 * p_114227_, p_114226_, 0.0F)
         .m_6122_(255, 255, 255, 0)
         .m_7421_(xOffset + 1.0F, yOffset + 1.0F)
         .m_86008_(OverlayTexture.f_118083_)
         .m_85969_(240)
         .m_252939_(p_114092_, 0.0F, -1.0F, 0.0F)
         .m_5752_();
   ...
   protected void setupRotations(EntityFarseer farseer, PoseStack matrixStackIn, float f1, float f2, float f3) {
      ...
      if (this.m_5936_(farseer)) {
         f2 += (float)(Math.cos(farseer.f_19797_ * 3.25) * Math.PI * 0.4F);
      }
      ...
      if (farseer.f_20919_ > 0) {
         float f = (farseer.f_20919_ + f3 - 1.0F) / 20.0F * 1.6F;
         f = Mth.m_14116_(f);
         if (f > 1.0F) {
            f = 1.0F;
         }

         matrixStackIn.m_252781_(Axis.f_252436_.m_252977_(f * this.m_6441_(farseer) * invCameraAmount));
      } else if (m_194453_(farseer)) {
         matrixStackIn.m_85837_(0.0, farseer.m_20206_() + 0.1F, 0.0);
   ...
   class LayerOverlay extends RenderLayer<EntityFarseer, ModelFarseer> {
      ...
      public void render(
         ...
         if (entitylivingbaseIn.getAnimation() == EntityFarseer.ANIMATION_EMERGE) {
            VertexConsumer ivertexbuilder = bufferIn.m_6299_(RenderType.m_110458_(RenderFarseer.TEXTURE_CLAWS));
            ((ModelFarseer)this.m_117386_())
