...
public abstract class LayerVoidWormGlow<T extends LivingEntity> extends RenderLayer<T, EntityModel<T>> {
   ...
   public void render(
      ...
      if (this.isGlowing(worm) || special) {
         if (special) {
            ...
            VertexConsumer consumer = AMRenderTypes.createMergedVertexConsumer(
               bufferIn.m_6299_(AMRenderTypes.VOID_WORM_PORTAL_OVERLAY), bufferIn.m_6299_(RenderType.m_110458_(texture))
            );
            ...
            this.m_117386_()
               .m_7695_(matrixStackIn, bufferIn.m_6299_(RenderType.m_110488_(texture)), 240, LivingEntityRenderer.m_115338_(worm, 1.0F), 1.0F, 1.0F, 1.0F, f);
         }
      }
   }
   ...
   private boolean isSpecialRenderer(ResourceLocation resourceLocation) {
      if (this.mcmetaData.containsKey(resourceLocation)) {
         ...
         try {
            VoidWormMetadataSection section = resource.m_215509_().m_214059_(VoidWormMetadataSection.SERIALIZER).orElse(new VoidWormMetadataSection());
